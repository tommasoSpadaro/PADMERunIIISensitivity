...
statisticalTreatmentTH::~statisticalTreatmentTH(){
  ...
  delete fLBHisto;
  for (int i=0; i<3; i++){
    if (fitfunB[i] != nullptr) delete fitfunB[i];
    if(fitfunEpsOverB[i]  != nullptr) delete fitfunEpsOverB[i] ;
  }
  
  fLSBHisto.clear(); 
  fQobsHisto.clear(); 
  for (int i=0; i<2; i++) fAsimov_a[i].clear();
  fQobs_a.clear();
  fQmuProfile.clear();
  fPprimeProfile.clear();
  ...
  for (int i=0; i<2; i++) fPullBFitOnBToy[i].clear();
  for (int i=0; i<2; i++) fPullSBFitOnBToy[i].clear();
  fPullBFitOnSBToy.clear();
  fPullSBFitOnSBToy.clear();
  delete fGraphBFitOnBToy;
  fGraphSBFitOnBToy.clear();
  cout << "..." << endl;
...
//  }
  ...
  for (int i=0; i<potGraph->GetN(); i++){
    ...
    // retrieve integrated pots
    fObservables.POTIntObs.push_back(potIntGraph->GetY()[i]/1E10); // POTInts observed in 1E10 units        
    
    if (fConfigPtr->GetObsLimitMode()){
      fObservables.NObs.push_back(nobsGraph->GetY()[i]);
    } else {
  ...
  if (fConfigPtr->GetObsLimitMode()){
    std::cout << "Nobs from input file ";
    for(int i=0; i<fObservables.NObs.size(); i++){ std::cout << " " << fObservables.NObs.at(i);}
    std::cout << endl;
  }
  ...
  fPotIntGraphUsed  = new TGraph(fObservables.SqrtsObs.size()); fPotIntGraphUsed  ->SetName("fPotIntGraphUsed");
  for (uint i=0; i<3; i++){ // scan1, 2, all
    ...
    fBkgGraphUsedScan[i]  = new TGraphErrors(); fBkgGraphUsedScan[i]  ->SetName(Form("fBkgGraphUsedScan_period_%d",i));
  }
  for (uint i=0; i<fObservables.SqrtsObs.size(); i++){
    ...
    fPotIntGraphUsed  ->SetPoint(i,fObservables.SqrtsObs.at(i),fObservables.POTIntObs.at(i));
    ...
    fBkgGraphUsedScan[period]  ->SetPoint(noldpts,fObservables.SqrtsObs.at(i),fObservables.BkgObs.at(i));
    fBkgGraphUsedScan[period]  ->SetPointError(noldpts,0.,fExpectedErrors.BkgErr.at(i));
    ...
    fBkgGraphUsedScan[2]  ->SetPoint(nallpts,fObservables.SqrtsObs.at(i),fObservables.BkgObs.at(i));
    fBkgGraphUsedScan[2]  ->SetPointError(nallpts,0.,fExpectedErrors.BkgErr.at(i));
  }
  ...
  for (int i=0; i<3; i++){ // scan1, scan2, scan all
    ...
    TFitResultPtr fitres = fBkgGraphUsedScan[i]->Fit(fitfunB[i],"ESQ");
    for (int j=0; j<2; j++) {
      fObservables.BkgPerPOTVsSqrtsParObs[i][j] = fitfunB[i]->GetParameter(j);
      fExpectedErrors.BkgPerPOTVsSqrtsParErr[i][j] = fitfunB[i]->GetParError(j);
    }
    
    TMatrixDSym cors(2);
    cors = fitres->GetCorrelationMatrix();
    fExpectedErrors.BkgPerPOTVsSqrtsP0P1Corr[i] = cors[0][1];
    std::cout << "Init: determination of B/POT vs sqrt(s) parameters (pol1 fit): scan " << i << 
    " par0 = " << fObservables.BkgPerPOTVsSqrtsParObs[i][0] << " +- " << fExpectedErrors.BkgPerPOTVsSqrtsParErr[i][0] <<
    " par1 = " << fObservables.BkgPerPOTVsSqrtsParObs[i][1] << " +- " << fExpectedErrors.BkgPerPOTVsSqrtsParErr[i][1] <<
    " corr = " << fExpectedErrors.BkgPerPOTVsSqrtsP0P1Corr[i] << " chi^2 prob = " << fitres->Prob() << endl;
  }  
  ...
  fObservables.POTSlopeCorrectionObs = fConfigPtr->GetPOTSlopeCorrectionObs(); 
  fExpectedErrors.POTSlopeCorrectionErr = fConfigPtr->GetPOTSlopeCorrectionErr(); 
  ...
  for (int i=0; i<2; i++) {
	...
	fInvCholeTransBkgVsSqrtsScan1st[i][j] = 1;
	fInvCholeTransBkgVsSqrtsScan2nd[i][j] = 1;
	fInvCholeTransBkgVsSqrtsScanAll[i][j] = 1;
	...
	fInvCholeTransBkgVsSqrtsScan1st[i][j] = 0;
	fInvCholeTransBkgVsSqrtsScan2nd[i][j] = 0;
	fInvCholeTransBkgVsSqrtsScanAll[i][j] = 0;
  ...
  if (fConfigPtr->GetCorrectBkgBias()){
    ...
    double elements[4] = {
      corrFactor*corrFactor*fExpectedErrors.BkgBiasErrP0*fExpectedErrors.BkgBiasErrP0,
      corrFactor*corrFactor*fExpectedErrors.BkgBiasErrP0*fExpectedErrors.BkgBiasErrP1*fExpectedErrors.BkgBiasErrP0P1Corr,
      corrFactor*corrFactor*fExpectedErrors.BkgBiasErrP0*fExpectedErrors.BkgBiasErrP1*fExpectedErrors.BkgBiasErrP0P1Corr,
      corrFactor*corrFactor*fExpectedErrors.BkgBiasErrP1*fExpectedErrors.BkgBiasErrP1};
    ...
    for (int i=0; i<2; i++) {
      ...
      for (int j=0; j<2; j++) {
	fInvCholeTransBkgBias[i][j] = invCholeTrans[i][j]/corrFactor;
	std::cout << " " << fInvCholeTransBkgBias[i][j];
      }
      std::cout << endl;
    }
  }
  ...
  if (fConfigPtr->GetAssumeBkgOverPotCurve()){ 
	...
	corrFactor*corrFactor*fExpectedErrors.BkgPerPOTVsSqrtsParErr[q][0]*fExpectedErrors.BkgPerPOTVsSqrtsParErr[q][1]*fExpectedErrors.BkgPerPOTVsSqrtsP0P1Corr[q],
	corrFactor*corrFactor*fExpectedErrors.BkgPerPOTVsSqrtsParErr[q][0]*fExpectedErrors.BkgPerPOTVsSqrtsParErr[q][1]*fExpectedErrors.BkgPerPOTVsSqrtsP0P1Corr[q],
	corrFactor*corrFactor*fExpectedErrors.BkgPerPOTVsSqrtsParErr[q][1]*fExpectedErrors.BkgPerPOTVsSqrtsParErr[q][1]};
      TMatrixDSym cov(2,elements);
      TMatrixDSym invCov = cov.Invert(); // apparently this inverts cov as well
      chole->SetMatrix(invCov);   // use Cholesky decomposition
      chole->Decompose();         // use Cholesky decomposition
	...
	std::cout << "Choleski Matrix, row = " << i;
	for (int j=0; j<2; j++) {
	  if (q==0)        fInvCholeTransBkgVsSqrtsScan1st[i][j] = invCholeTrans[i][j]/corrFactor;
	  else if (q==1)   fInvCholeTransBkgVsSqrtsScan2nd[i][j] = invCholeTrans[i][j]/corrFactor;
	  else             fInvCholeTransBkgVsSqrtsScanAll[i][j] = invCholeTrans[i][j]/corrFactor;
	  std::cout << " " << invCholeTrans[i][j]/corrFactor;
	}
	std::cout << endl;
      }
    }
  ...
  for (int sbidx = 0; sbidx<2; sbidx++){
    fInitialPars[sbidx].clear();
    fInitialErr[sbidx].clear();
  }
  initFitters(false,fTheta_S); // SB fitter
  initFitters(true,fTheta_B); // B fitter
...
nuisancePars statisticalTreatmentTH::InitNuisanceToObservables(observables obs){ 
  ...
  for (int q=0; q<3; q++){
    for (int i=0; i<2; i++){
      nuis.BkgPerPOTVsSqrtsParTrue[q][i] = obs.BkgPerPOTVsSqrtsParObs[q][i];
    }
  }
  nuis.POTSlopeCorrectionTrue = obs.POTSlopeCorrectionObs;
...
void statisticalTreatmentTH::initHistograms(){
  ...
  for (int k=0; k<2; k++) { // 90%CL, 95%CL
    for (int i=0; i<2; i++) { // 1-sigma and 2-sigma bands
      ...
      asi->SetName(Form("Asimov_cl%d_sigma%d",90+5*k,i+1));
      fAsimov_a[k].push_back(asi);
    }
  }
  ...
  for (uint im = 0; im < (uint)fNMassBins; im++){
    ...
    grafo->SetName(Form("Asimov_qobs_mass_%d",im));
    fQobs_a.push_back(grafo);

    // qmu as in Eq.6 of Frequentist_Limit_Recommendation
    grafo = new TGraph();
    grafo->SetName(Form("qmuprofile_mass_%d",im));
    fQmuProfile.push_back(grafo);
    grafo = new TGraph();
    grafo->SetName(Form("pprimemuprofile_mass_%d",im));
    fPprimeProfile.push_back(grafo);
  }
...
void statisticalTreatmentTH::ReadOutput(TString filename){
  ...
  fCLsTree->SetBranchAddress("NsValid",&fCLsInfo.NsValid);
  fCLsTree->SetBranchAddress("Pb",&fCLsInfo.Pb);
  fCLsTree->SetBranchAddress("NbValid",&fCLsInfo.NbValid);
  ...
  TGraph* limit90[2];
  TGraph* limit95[2];
  TString limname[2] = {"","CLsb"};
  for (int i=0; i<2; i++){ // CLs, CLs+b
    limit90[i] = new TGraph(); 
    limit95[i] = new TGraph(); 
    limit90[i]->SetName(Form("Limit90%s",limname[i].Data()));
    limit95[i]->SetName(Form("Limit95%s",limname[i].Data()));
  }
  
  vector<TGraphAsymmErrors*> gveVSCLs[2];
  TGraphAsymmErrors* gveVSCLsLoc[2];

  double massOld = -1;
  const double clsthreshold = 0.7;
  ...
  // fit function: direct and inverse
  ...
  for (int i=0; i< fCLsTree->GetEntries(); i++){
    ...
    // evaluate cls and errors
    if ((fCLsInfo.Ps*fCLsInfo.NsValid > fCLsInfo.NsValid) || (fCLsInfo.Pb*fCLsInfo.NbValid > fCLsInfo.NbValid)){
      cout << "Efficiency problem bin " << " " << 
	fCLsInfo.Ps << " " << fCLsInfo.NsValid << " " <<
	fCLsInfo.Pb << " " << fCLsInfo.NbValid << " " << fCLsInfo.im << " " << fCLsInfo.ig << endl;
    }

    TEfficiency effo;
    effo.SetConfidenceLevel(0.68);
    effo.SetStatisticOption(option);
    effo.SetTotalEvents(1,fCLsInfo.NsValid);//fNumberOfGenerations);
    effo.SetTotalEvents(2,fCLsInfo.NbValid);//fNumberOfGenerations);
    effo.SetPassedEvents(1,int(fCLsInfo.Ps*fCLsInfo.NsValid));
    effo.SetPassedEvents(2,int(fCLsInfo.Pb*fCLsInfo.NbValid));
    double ps = effo.GetEfficiency(1);
    double pserrup = effo.GetEfficiencyErrorUp(1);
    double pserrdw = effo.GetEfficiencyErrorLow(1);
    double pb = effo.GetEfficiency(2);
    double pberrup = effo.GetEfficiencyErrorUp(2);
    double pberrdw = effo.GetEfficiencyErrorLow(2);
    double cln[2] = {ps/(1-pb),ps};
    double clnerrup[2] = {TMath::Sqrt(pow(pserrup/(1-pb),2) + pow(ps/(1-pb)/(1-pb)*pberrup,2)), pserrup};
    double clnerrdw[2] = {TMath::Sqrt(pow(pserrdw/(1-pb),2) + pow(ps/(1-pb)/(1-pb)*pberrdw,2)), pserrdw};
    ...
    if (massOld == -1) { // first mass
      ...
      for (int j = 0; j<2; j++){
	gveVSCLsLoc[j] = new TGraphAsymmErrors();
	gveVSCLsLoc[j]->SetName(Form("gveVSCLsLoc%f_graph%s",massOld,limname[j].Data()));
	gveVSCLsLoc[j]->SetPoint(0,fCLsInfo.gven,cln[j]);//invece di fCLsInfo.CLs,fCLsInfo.gven);
	gveVSCLsLoc[j]->SetPointError(0,0.,0.,clnerrdw[j],clnerrup[j]);
      }
    }
    else {
	...
	if (fVerbosity > 2) {
	  ...
	  for (int j = 0; j< gveVSCLsLoc[0]->GetN(); j++){
	    std::cout << "Point: " << j << " gven = " << gveVSCLsLoc[0]->GetX()[j] << " CLs = " << gveVSCLsLoc[0]->GetY()[j]<< " Ps = " << gveVSCLsLoc[1]->GetY()[j] << std::endl;
	  }
	}

	// fill limits, if we can interpolate the CLs
	
	for (int j = 0; j<2; j++) gveVSCLsLoc[j]->Sort(); // sort graphs by gven

	if (gveVSCLsLoc[0]->GetY()[gveVSCLsLoc[0]->GetN()-1] < clsthreshold) {
	  clsfun->SetParameter(0,0.5);
	  clsfun->SetParameter(1,2E-4);
	  clsfun->SetParameter(2,2E-4);

	  for (int j = 0; j < 2; j++) { // CLs, CLsb
	    TFitResultPtr fitptr = gveVSCLsLoc[j]->Fit("clsfun","MS","",0.5E-4,1E-3);
	    if (fitptr.Get()->IsValid()) { // fit OK
	      for (int q = 0; q<3; q++) clsfunInv->SetParameter(q,clsfun->GetParameter(q));	    
	      int nn = limit90[j]->GetN();
	      limit90[j]->SetPoint(nn,massOld, clsfunInv->Eval(0.1));
	      limit95[j]->SetPoint(nn,massOld, clsfunInv->Eval(0.05));
	    } else { // fit not OK
	      cout << "Fit " << massOld << " for variable " << j << " Not OK " << (int( fitptr.Get()->Status())) << endl;
	      TGraphAsymmErrors* gveVSCLsLocInv = new TGraphAsymmErrors();
	      for (int q = 0; q<gveVSCLsLoc[j]->GetN(); q++){ // invert X and Y
		gveVSCLsLocInv->SetName(Form("gveVSCLsLoc%f_graphInv",massOld));
		gveVSCLsLocInv->SetPoint(q,gveVSCLsLoc[j]->GetY()[q],gveVSCLsLoc[j]->GetX()[q]);
		gveVSCLsLocInv->SetPointError(q,gveVSCLsLoc[j]->GetEYlow()[q],gveVSCLsLoc[j]->GetEYhigh()[q],0.,0.);
	      }
	      // fill limit plots done with CLs
	      int nn = limit90[j]->GetN();
	      limit90[j]->SetPoint(nn,massOld, gveVSCLsLocInv->Eval(0.1));
	      limit95[j]->SetPoint(nn,massOld, gveVSCLsLocInv->Eval(0.05)); 
	      delete gveVSCLsLocInv;
	    }
	  }
	}
 	// save graph for CLs vs gve at given mass 
	for (int j = 0; j < 2; j++) gveVSCLs[j].push_back(gveVSCLsLoc[j]);
	...
	for (int j = 0; j < 2; j++) {
      ...
      // fill point
      int ipt = gveVSCLsLoc[0]->GetN();
      for (int j = 0; j < 2; j++) {
	gveVSCLsLoc[j]->SetPoint(ipt,fCLsInfo.gven,cln[j]);//invece di fCLsInfo.CLs,fCLsInfo.gven);
	gveVSCLsLoc[j]->SetPointError(ipt,0.,0.,clnerrdw[j],clnerrup[j]);
      }
    }
  }
  // fill limits, if we can interpolate
  if (fCLsTree->GetEntries()){
    for (int j = 0; j < 2; j++) gveVSCLsLoc[j]->Sort();
	...
	if (fitptr.Get()->IsValid()) { // fit OK
	  ...
	  cout << "Last fit " << massOld << " for variable " << j << " Not OK " << (int( fitptr.Get()->Status())) << endl;
	  TGraphAsymmErrors* gveVSCLsLocInv = new TGraphAsymmErrors();
	  for (int q = 0; q<gveVSCLsLoc[0]->GetN(); q++){ // invert X and Y
	    gveVSCLsLocInv->SetName(Form("gveVSCLsLoc%f_graphInv",massOld));
	    gveVSCLsLocInv->SetPoint(q,gveVSCLsLoc[j]->GetY()[q],gveVSCLsLoc[j]->GetX()[q]);
	    gveVSCLsLocInv->SetPointError(q,gveVSCLsLoc[j]->GetEYlow()[q],gveVSCLsLoc[j]->GetEYhigh()[q],0.,0.);
	  }
  ...
  for (int j = 0; j < 2; j++) {
    limit90[j]->Write();
    limit95[j]->Write();
    for (uint i=0; i<gveVSCLs[j].size(); i++){
      gveVSCLs[j].at(i)->Write();
    }
  }
  limitRL90->Write();
  limitRL95->Write();
  ...
  for (int j = 0; j < 2; j++)  gveVSCLs[j].clear();
}


void statisticalTreatmentTH::initFitters(bool bonly, nuisancePars theta){
  ...
  // 19+3npts             BkgFitP0 scan period 0       <--- parameter 0 of a pol1 fit for B/POT vs sqrt(s)
  // 20+3npts             BkgFitP1 scan period 0       <--- parameter 1 of a pol1 fit for B/POT vs sqrt(s)
  // 21+3npts             BkgFitP0 scan period 1       <--- parameter 0 of a pol1 fit for B/POT vs sqrt(s)
  // 22+3npts             BkgFitP1 scan period 1       <--- parameter 1 of a pol1 fit for B/POT vs sqrt(s)
  // 23+3npts             BkgFitP0 scan period all     <--- parameter 0 of a pol1 fit for B/POT vs sqrt(s)
  // 24+3npts             BkgFitP1 scan period all     <--- parameter 1 of a pol1 fit for B/POT vs sqrt(s)
  // 25+3npts             UsePOTSlopeCorrection <---- always fixed, if 1 -> will use potSlopeCorrection vs POTIntegrated
  // 26+3npts             POTSlopeCorrectionTrue       <--- fixed to 0 if above parameter != 1
  
  // total number of parameters passed to the likelihood
  int npars = 27+3*npts; // 8 always fixed, 14 + 3 * npts depending on the setup 
  ...
  const double stepFac = 0.01;
  parnames[0] = "IsBonlyFit"; parinput[0] = bonly ? 1:0  ; parstep[0] = 0;
  parnames[1] = "mass"      ; parinput[1] = fMassMin  ; parstep[1] = 0; 
  parnames[2] = "coupling"  ; parinput[2] = fgveMin   ; parstep[2] = 0; 
  for (int i=0; i<npts; i++){
    parnames[3+i] = Form("POTTrue_%d",i)        ; parinput[3+i] = theta.POTTrue.at(i)                 ; parstep[3+i] = fExpectedErrors.POTLocalErr.at(i)*stepFac;
    parnames[3+npts+i] = Form("SigEffTrue_%d",i); parinput[3+npts+i] = theta.SignalEffiLocalTrue.at(i); parstep[3+npts+i] = fExpectedErrors.SignalEffiLocalErr.at(i)*stepFac;    
  }
  parnames[3+2*npts] = "SigPeakYieldTrue"; parinput[3+2*npts] = theta.SignalPeakYieldTrue        ; parstep[3+2*npts] = fExpectedErrors.SignalPeakYieldErr*stepFac;
  parnames[4+2*npts] = "SigLorentzWTrue" ; parinput[4+2*npts] = theta.SignalLorentzianWidthTrue  ; parstep[4+2*npts] = fExpectedErrors.SignalLorentzianWidthErr*stepFac;
  parnames[5+2*npts] = "BESTrue"         ; parinput[5+2*npts] = theta.BESTrue                         ; parstep[5+2*npts] = fExpectedErrors.BESErr*stepFac;
  parnames[6+2*npts] = "POTScaleTrue"    ; parinput[6+2*npts] = theta.POTScaleTrue                    ; parstep[6+2*npts] = fExpectedErrors.POTScaleErr*stepFac;
  for (int i=0; i<npts; i++){
    parnames[7+2*npts+i] = Form("BkgTruePerPOT_%d",i); parinput[7+2*npts+i] = theta.BkgTrue.at(i)     ; parstep[7+2*npts+i] = fExpectedErrors.BkgErr.at(i)*stepFac;
  }
  parnames[7+3*npts] = "CorrectBkgBias"        ; parinput[7+3*npts] = fConfigPtr->GetCorrectBkgBias()        ; parstep[7+3*npts] = 0;    
  parnames[8+3*npts] = "BkgBiasTrueP0"         ; parinput[8+3*npts] = theta.BkgBiasTrueP0                 ; parstep[8+3*npts] = fExpectedErrors.BkgBiasErrP0*stepFac;
  parnames[9+3*npts] = "BkgBiasTrueP1"         ; parinput[9+3*npts] = theta.BkgBiasTrueP1                 ; parstep[9+3*npts] = fExpectedErrors.BkgBiasErrP1*stepFac;
  parnames[10+3*npts]= "AssumeEffiOverBkgCurve"; parinput[10+3*npts]= fConfigPtr->GetAssumeEffiOverBkgCurve(); parstep[10+3*npts]= 0;    
  parnames[11+3*npts]= "EffiOverBkgTrueP0_0"   ; parinput[11+3*npts]= theta.EffiOverBkgTrueP0[0]          ; parstep[11+3*npts]= fExpectedErrors.EffiOverBkgErrP0[0]*stepFac;    
  parnames[12+3*npts]= "EffiOverBkgTrueP1_0"   ; parinput[12+3*npts]= theta.EffiOverBkgTrueP1[0]          ; parstep[12+3*npts]= fExpectedErrors.EffiOverBkgErrP1[0]*stepFac;    
  parnames[13+3*npts]= "EffiOverBkgTrueP0_1"   ; parinput[13+3*npts]= theta.EffiOverBkgTrueP0[1]          ; parstep[13+3*npts]= fExpectedErrors.EffiOverBkgErrP0[1]*stepFac;    
  parnames[14+3*npts]= "EffiOverBkgTrueP1_1"   ; parinput[14+3*npts]= theta.EffiOverBkgTrueP1[1]          ; parstep[14+3*npts]= fExpectedErrors.EffiOverBkgErrP1[1]*stepFac;    
  parnames[15+3*npts]= "EffiOverBkgTrueP0_2"   ; parinput[15+3*npts]= theta.EffiOverBkgTrueP0[2]          ; parstep[15+3*npts]= fExpectedErrors.EffiOverBkgErrP0[2]*stepFac;    
  parnames[16+3*npts]= "EffiOverBkgTrueP1_2"   ; parinput[16+3*npts]= theta.EffiOverBkgTrueP1[2]          ; parstep[16+3*npts]= fExpectedErrors.EffiOverBkgErrP1[2]*stepFac;    
  ...
  parnames[19+3*npts]= "BkgOverPotTrueP0_0"    ; parinput[19+3*npts]= theta.BkgPerPOTVsSqrtsParTrue[0][0]    ; parstep[19+3*npts]= fExpectedErrors.BkgPerPOTVsSqrtsParErr[0][0]*stepFac;
  parnames[20+3*npts]= "BkgOverPotTrueP1_0"    ; parinput[20+3*npts]= theta.BkgPerPOTVsSqrtsParTrue[0][1]    ; parstep[20+3*npts]= fExpectedErrors.BkgPerPOTVsSqrtsParErr[0][1]*stepFac;
  parnames[21+3*npts]= "BkgOverPotTrueP0_1"    ; parinput[21+3*npts]= theta.BkgPerPOTVsSqrtsParTrue[1][0]    ; parstep[21+3*npts]= fExpectedErrors.BkgPerPOTVsSqrtsParErr[1][0]*stepFac;
  parnames[22+3*npts]= "BkgOverPotTrueP1_1"    ; parinput[22+3*npts]= theta.BkgPerPOTVsSqrtsParTrue[1][1]    ; parstep[22+3*npts]= fExpectedErrors.BkgPerPOTVsSqrtsParErr[1][1]*stepFac;
  parnames[23+3*npts]= "BkgOverPotTrueP0_2"    ; parinput[23+3*npts]= theta.BkgPerPOTVsSqrtsParTrue[2][0]    ; parstep[23+3*npts]= fExpectedErrors.BkgPerPOTVsSqrtsParErr[2][0]*stepFac;
  parnames[24+3*npts]= "BkgOverPotTrueP1_2"    ; parinput[24+3*npts]= theta.BkgPerPOTVsSqrtsParTrue[2][1]    ; parstep[24+3*npts]= fExpectedErrors.BkgPerPOTVsSqrtsParErr[2][1]*stepFac;
  parnames[25+3*npts]= "UsePOTSlopeCorrection" ; parinput[25+3*npts]= fConfigPtr->GetUsePOTSlopeCorrection() ; parstep[25+3*npts]= 0;
  parnames[26+3*npts]= "POTSlopeCorrectionTrue"; parinput[26+3*npts]= theta.POTSlopeCorrectionTrue           ; parstep[26+3*npts]= fExpectedErrors.POTSlopeCorrectionErr*stepFac;

   
  // assign which fitter must be initialised
  ...
  fitter->SetFCN(npars, fLikeli); // flag_sb/b, mass, gve, signalPeak, LorentzianW, BES, potscale, 3 array: pottrue, signaleffitrue, bkgtrue
  for (int ip=0; ip < npars; ip++) {
    fitter->Config().ParSettings(ip) = ROOT::Fit::ParameterSettings(parnames[ip].Data(),parinput[ip],parstep[ip]);    
  }


  if (fInitialPars[sbidx].size() == 0){
    for (int ip=0; ip < npars; ip++) {
      fInitialPars[sbidx].push_back(parinput[ip]);
      fInitialErr[sbidx].push_back(parstep[ip]/stepFac);
    }
  }
  // parameters which are always fixed
  fitter->Config().ParSettings(0).Fix(); // used to define S+B or B fits
  fitter->Config().ParSettings(1).Fix(); // mass is always fixed
  fitter->Config().ParSettings(2).Fix(); // coupling is always fixed
  fitter->Config().ParSettings(7+3*npts).Fix(); // flag to correct the N2cl/pot/(bkg/pot) bias 
  fitter->Config().ParSettings(10+3*npts).Fix(); // flag to assume the Effi/(Bkg/pot) curve (options are 0, 1, 2)
  fitter->Config().ParSettings(17+3*npts).Fix(); // flag to apply a straight fit (options are 0, 1, 2)
  fitter->Config().ParSettings(18+3*npts).Fix(); // flag to apply a fit to B/POT vs s
  fitter->Config().ParSettings(25+3*npts).Fix(); // flag to apply the POTSlope correction vs integrated pot
  ...
  if (fConfigPtr->GetCorrectBkgBias()){ // USE the bias curve, therefore remove the potscale parameter, substituted by the curve
    fitter->Config().ParSettings(6+2*npts).SetValue(1);
    fitter->Config().ParSettings(6+2*npts).Fix();
  } else { // DO NOT USE the bias curve
    for (uint i=0; i<2; i++) {
      fitter->Config().ParSettings(8+3*npts+i).SetValue(1-i); // Set P0=1, P1=0 of the bias curve
      fitter->Config().ParSettings(8+3*npts+i).Fix();         // fix P0,P1 of the bias curve
    }
  }

  // Parameterisation of the B/Pot vs sqrt(s)
  if (fConfigPtr->GetAssumeBkgOverPotCurve()){  
    for (uint i=0; i<(uint)npts; i++)              fitter->Config().ParSettings(7+2*npts+i).Fix(); // fix B point-by-point
    if (fConfigPtr->GetAssumeBkgOverPotCurve() == 1) {
      for (uint i=0; i<2; i++) {
	fitter->Config().ParSettings(23+3*npts+i).SetValue(1-i); // Set P0=1, P1=0 of the B/pot vs sqrt(s) curve for ALL SCAN
	fitter->Config().ParSettings(23+3*npts+i).Fix();         // fix P0,P1 of the B/pot vs sqrts curve        for ALL SCAN
      }
    } else { // fix parameters per scan
      for (uint j=0; j<2; j++) {
	for (uint i=0; i<2; i++) {
	  fitter->Config().ParSettings(19+3*npts+i+2*j).SetValue(1-i); // Set P0=1, P1=0 of the B/pot vs sqrt(s) curve for ALL SCAN
	  fitter->Config().ParSettings(19+3*npts+i+2*j).Fix();         // fix P0,P1 of the B/pot vs sqrts curve        for ALL SCAN
	}
      }
    }
  }
    ...
    for (uint j=0; j<3; j++) {
      for (uint i=0; i<2; i++) {
	fitter->Config().ParSettings(19+3*npts+i+2*j).SetValue(1-i); // Set P0=1, P1=0 of the B/pot vs sqrt(s) curve
	fitter->Config().ParSettings(19+3*npts+i+2*j).Fix();         // fix P0,P1 of the B/pot vs sqrts curve
      }
    }
  }

  if (!fConfigPtr->GetUsePOTSlopeCorrection()){ // if not use the POTSlopeCorrection vs POTInt, set to 0 and fix the corresponding parameter
    fitter->Config().ParSettings(26+3*npts).SetValue(0);
    fitter->Config().ParSettings(26+3*npts).Fix();
  }
  
  // straight fit mode 1: fit N2/(POT)
  if (fConfigPtr->GetStraightFitMode() == 1){
    for (uint i=0; i<(uint)npts; i++)              fitter->Config().ParSettings(3+i).Fix(); // fix POT point-by-point
  }
  // straight fit mode 2: fit N2/(POT B)
  if (fConfigPtr->GetStraightFitMode() == 2){
    for (uint i=0; i<(uint)npts; i++)              fitter->Config().ParSettings(3+i).Fix(); // fix POT point-by-point
    for (uint i=0; i<(uint)npts; i++)              fitter->Config().ParSettings(7+2*npts+i).Fix(); // fix B point-by-point
  }
  
  if (bonly){
    for (uint i=0; i<(uint)npts; i++)  fitter->Config().ParSettings(3+npts+i).Fix(); // fix signal effi parameters
    for (uint i=0; i< 3; i++)          fitter->Config().ParSettings(i+3+2*npts).Fix();         // fix the 3 signal shape parameters
    for (uint i=0; i< 6; i++)          fitter->Config().ParSettings(i+11+3*npts).Fix();        // fix the 6 effi/(bkg/pot) curve parameters
  }
  else {
    if (fConfigPtr->GetAssumeEffiOverBkgCurve()){ // USE the effi/(bkg/pot) curve

      for (uint i=0; i<(uint)npts; i++)  fitter->Config().ParSettings(3+npts+i).Fix(); // fix signal effi point-by-point, if they had been released

      if (fConfigPtr->GetAssumeEffiOverBkgCurve() == 1) { // fix effipars for scanAll
	for (uint i=0; i<2; i++) { // P0,P1
	  fitter->Config().ParSettings(15+3*npts+i).SetValue(1-i); // Set P0=1, P1=0 of the effi curve
	  fitter->Config().ParSettings(15+3*npts+i).Fix();         // fix P0,P1 of the effi curve
	}      
      }
      else { // fix the parameters of the period-level effi/bkg curves
	for (uint j=0; j<2; j++) { // scan subset
	  for (uint i=0; i<2; i++) { // P0,P1
	    fitter->Config().ParSettings(11+3*npts+i+2*j).SetValue(1-i); // Set P0=1, P1=0 of the effi curve
	    fitter->Config().ParSettings(11+3*npts+i+2*j).Fix();         // fix P0,P1 of the effi curve
	  }
	}
      }
  ...
  if (!fUseNuisance) { 
	...
	if (ip == 6+2*npts) continue; // leave potscale as only fit parameter
      ...
      fitter->Config().ParSettings(ip).Fix(); //fix all the parameters
    }
  }
  ...
  int freePars = 0;
  for (uint ip=0; ip < npars; ip++) {
    if (!fitter->Config().ParSettings(ip).IsFixed()) {
      freePars++;      
    }
  }
  std::cout << "StatisticalTreatment >> InitFitter with" <<
    ...
    " UsePOTSlopeCorrection = " << fConfigPtr->GetUsePOTSlopeCorrection() << 
...
void statisticalTreatmentTH::fillPullHistograms(bool sbfitmode, double massn, double gven, bool sbtoy, bool pseudob){ // 0->toy-of-toy, 1->pseudoevent
  //  cout << "Fill the fit pulls " << sbfitmode << " " << massn << " " << gven << " " << sbtoy << " " << pseudob << endl;

  int pseudo = pseudob? 1: 0;
  ...
  int im = (massn-fMassMin)/fMassStep + 0.5;

  int np=-1;
  for (uint ip=0; ip < fitter->Config().NPar(); ip++) {
    if (fitter->Config().ParSettings(ip).IsFixed()) continue;
    np++;

    double pullp = (fitter->Result().GetParams()[ip] - fInitialPars[sbidx].at(ip))/fInitialErr[sbidx].at(ip); 
    if (!sbfitmode && !sbtoy) {
      fPullBFitOnBToy[pseudo].at(np)->Fill(pullp); // b-fit on b-only toy
    } else if (!sbfitmode && sbtoy) {
      int idx = im + np*fNMassBins;
      fPullBFitOnSBToy.at(idx)->Fill(pullp,gven);  // b-fit on sb-toy
    } else if (sbfitmode && !sbtoy) { 	
      int idx = im + np*fNMassBins;
      fPullSBFitOnBToy[pseudo].at(idx)->Fill(pullp,gven); // sb-fit on b-only toy
    } else {
      int idx = im + np*fNMassBins;
      fPullSBFitOnSBToy.at(idx)->Fill(pullp,gven);        // sb-fit on sb-toy
    }
  }

  // retrieve variables for fit evaluation
  // calculate either N2/(POT B) x biasCorr(sqrt(s))  or N2/POT B x biasCorr(sqrt(s)) x (1 + S(m,g) gve^2 )

  int npts = fObservables.NObs.size();
  double signalPeakYieldTrue = fitter->Result().GetParams()[3+2*npts];
  double signalLorentzianWidthTrue  = fitter->Result().GetParams()[4+2*npts];
  double BESTrue   = fitter->Result().GetParams()[5+2*npts];
  double potScaleTrue = fitter->Result().GetParams()[6+2*npts];
  bool useBkgBias = fitter->Result().GetParams()[7+3*npts]; // use or not a pol1 for the bkg (in fact pot) bias
  double bkgBiasTrueP0 = fitter->Result().GetParams()[8+3*npts];
  double bkgBiasTrueP1 = fitter->Result().GetParams()[9+3*npts];
  int assumeEffiOverBkgCurve = fitter->Result().GetParams()[10+3*npts];
  double effiOverBkgTrueP0[3];
  double effiOverBkgTrueP1[3];
  for (int i=0; i<3; i++){
    effiOverBkgTrueP0[i] = fitter->Result().GetParams()[11+3*npts+2*i];
    effiOverBkgTrueP1[i] = fitter->Result().GetParams()[12+3*npts+2*i];
  }
  int straightFitMode = fitter->Result().GetParams()[17+3*npts];
  bool bkgFitMode = fitter->Result().GetParams()[18+3*npts];
  double bkgFitP0 = fitter->Result().GetParams()[19+3*npts];
  double bkgFitP1 = fitter->Result().GetParams()[20+3*npts];
  //  int potTrueIdx = 3; // presently not used because we are plotting the fit result in terms of fit values for N2/pot or N2/pot/B
  int effiTrueIdx = 3 + npts;
  int bkgTrueIdx = 7 + 2*npts;

  bool usePotSlopeCorrection = fitter->Result().GetParams()[25+3*npts];
  double potSlopeCorrection = fitter->Result().GetParams()[26+3*npts];

  // at the moment treat only the straighfitmode = 1 and bkgfitmode = 1 and assumeeffioverbkgcurve = 2

  int ig = (gven-fgveMin)/fgveStep + 0.5;
  for (int i=0; i< npts; i++){
    double biasCorr = 0;
    if (useBkgBias) biasCorr = bkgBiasTrueP0 + bkgBiasTrueP1*(fObservables.SqrtsObs.at(i)-SQRTSMID);
    else            biasCorr = potScaleTrue;

    double bkgPerPot = 0;
    if (bkgFitMode) bkgPerPot = bkgFitP0 + bkgFitP1*(fObservables.SqrtsObs.at(i)-SQRTSMID);
    else            bkgPerPot = fitter->Result().GetParams()[bkgTrueIdx+i];

    double potSlopeCorr = 1;
    if (usePotSlopeCorrection) potSlopeCorr = 1./(1.+potSlopeCorrection*fObservables.POTIntObs.at(i)/fObservables.POTIntObs.at(npts-1));
    
    double effiOverBkg = 0;
    if (assumeEffiOverBkgCurve) {
      int iperiod = 2;
      if (assumeEffiOverBkgCurve == 1) iperiod = fObservables.ScanPeriod.at(i); //0 or 1
      effiOverBkg = effiOverBkgTrueP0[iperiod] + effiOverBkgTrueP1[iperiod]*(fObservables.SqrtsObs.at(i)-SQRTSMID);
    }
    else effiOverBkg = fitter->Result().GetParams()[effiTrueIdx+i]/bkgPerPot;

    double sc = 0;
    if (sbfitmode) sc = gven*gven*Likelihood::SignalShape(signalPeakYieldTrue,BESTrue,signalLorentzianWidthTrue,massn,fObservables.SqrtsObs.at(i));

    double valfit = 0;
    if (straightFitMode == 1) valfit = biasCorr*bkgPerPot*potSlopeCorr; 
    else                      valfit = biasCorr*potSlopeCorr;
    if (sbfitmode)            valfit *= (1.+sc*effiOverBkg);
    
    if (!sbfitmode && !sbtoy && pseudo == 1) {
      int nptold = fGraphBFitOnBToy->GetN();
      fGraphBFitOnBToy->SetPoint(nptold,fObservables.SqrtsObs.at(i),valfit); // b-fit on b-only toy
    } else if (sbfitmode && !sbtoy && pseudo == 1) { 	
      int idx = im + ig*fNMassBins;
      if (im < 0 || ig < 0 || im >= fNMassBins || ig >= fNgveBins || idx >= fGraphSBFitOnBToy.size()) std::cerr << "fill wrong idx graph sbfitmode && !sbtoy " << im << " " << ig << endl;
      int nptold = fGraphSBFitOnBToy.at(idx)->GetN();
      fGraphSBFitOnBToy.at(idx)->SetPoint(nptold,fObservables.SqrtsObs.at(i),valfit); // sb-fit on b-only toy
    }
  }
  //  cout << "Done with fit pulls " << sbfitmode << " " << massn << " " << gven << " " << sbtoy << " " << pseudob << endl;
}

void statisticalTreatmentTH::initPullHistograms(){
  double* gveEdges = new double[fNgveBins+1];
  for (uint ig = 0; ig < (uint)fNgveBins+1; ig++) gveEdges[ig] = fgveMin + fgveStep*ig; //TMath::Power(fgveMin*fgveMin*fgveMin*fgveMin + ig*fgveStep,0.25);
  
  ROOT::Fit::Fitter* fitter;
  
  for (int isb=0; isb<2; isb++){ // bfit, sbfit

    fitter = &fFitterB;
    if (isb==1) fitter = &fFitterSB;
...
// graph with fit results

// b-only fit     
  fGraphBFitOnBToy =  new TGraph();
  fGraphBFitOnBToy->SetName("BFitOnBToyGraph_pseudoEvts");
 
// sb-fit
  for (uint ig = 0; ig < fNgveBins; ig++){
    double gven = fgveMin + ig*fgveStep;
    for (uint im = 0; im < fNMassBins; im++){
      double massn = fMassMin + im*fMassStep;
      TGraph* grafo = new TGraph();
      grafo->SetName(Form("SBFitOnBToyGraph_im%d_ig%d_mass%f_gven%f_pseudoEvts",im,ig,massn,gven));
      fGraphSBFitOnBToy.push_back(grafo);
    }
  }      
  cout << "BookPull: booked SBFitOnBToyGraph vector of tgraphs " <<  fGraphSBFitOnBToy.size() << endl;
...
void statisticalTreatmentTH::EvaluateExpectedLimit(){
  ...
  // asimptotic median, +-1 sigma, +-2sigma bands
  double asimLim[5][2];
  for (int j=0; j<2; j++){ // 90% CL, 95%CL
    for (int i=0; i<5; i++) asimLim[i][j] = PhiFunInv(1.-(0.10-0.05*j)*PhiFun(i-2)) + (i-2);    
  }
  
  // define temporary structures to be stored in memory 
  double** Qobs = new double*[fNMassBins];
  double** Qobs_a = new double*[fNMassBins]; // asimov data set
  double* mu95_a = new double[fNMassBins]; 
  double* mu90_a = new double[fNMassBins]; 
  ...
  nuisancePars** theta_S_a = new nuisancePars*[fNMassBins]; // asimov data set
  ...
  for (uint im = 0; im < (uint) fNMassBins; im++) {
    ...
    Qobs_a[im] = new double[fNgveBins];
    ...
    theta_S_a[im] = new nuisancePars[fNgveBins];
  ...
  SetObservablesAsimov(fTheta_B); // also the Nobs are set without fluctuations
  ...
  // evaluate Qobs(im,ig) = LSB(im,ig)-LB for each coupling and mass
  // evaluate Qmu = LSB(im,ig)-LSB(im,ig^(im))
    double lsbmin = 99999;
    int igmin = -1;
    for (uint ig = 0; ig < (uint) fNgveBins; ig++){
      ...
      theta_S_i[im][ig] = MaximizeL(fTheta_S,true,massn,gven,&LSB,&okfita);
      if (igmin == -1 || LSB < lsbmin) {
	lsbmin = LSB;
	igmin = ig;
      }
      Qobs_a[im][ig] = LSB;
    ...
    for (uint ig = 0; ig < (uint) fNgveBins; ig++) {
      double gven = fgveMin + ig*fgveStep; //TMath::Power(fgveMin*fgveMin*fgveMin*fgveMin + ig*fgveStep,0.25);      
      Qobs_a[im][ig] -= Qobs_a[im][igmin]; // qmu = -2log(mu, theta_doublehat) + 2log(mu_hat,theta_hat) 
      fQobs_a.at(im)->SetPoint(ig,gven,Qobs_a[im][ig]);
    }
  }
  ...
  for (uint i=0; i< (uint) fNumberOfGenerationsExpectedLimit; i++){
    ...
    if (i%10 == 0) cout << "Simulating pseudo data, event " << i << " / " << fNumberOfGenerationsExpectedLimit << endl;
    ...
    // set observables either from direct simulation or from input file
    if (fConfigPtr->GetObsLimitMode()) { 
      cout << "StatisticalTreatment - Retrieving original data in memory" << endl;
      fObservables = fObservablesInit; // do nothing: the NObs are already set
      std::cout << "Nobs as of now ";
      for(int i=0; i<fObservables.NObs.size(); i++){ std::cout << " " << fObservables.NObs.at(i);}
      std::cout << endl;
    } else {
    ...
    double LB;
    bool fitstatusB;
    nuisancePars theta_B_i = MaximizeL(fTheta_B,false,0.,0.,&LB,&fitstatusB);   // maximize Lb wrt nuisance pars, store "profiled" nuisance pars q_b
    fLBHisto->Fill(LB);
    if (fitstatusB) fillPullHistograms(false,0.,0.,false,true);  // bool sbfitmode, double massn, double gven, bool sbtoy, bool pseudo
	...
	double LSB;
	bool fitstatusSB;
	theta_S_i[im][ig] = MaximizeL(fTheta_S,true,massn,gven,&LSB,&fitstatusSB); // maximize Lsb, return "profiled" nuisance pars and via argument, Lsb    
	...
	if (fitstatusSB) fillPullHistograms(true,massn,gven,false,true);//bool sbfitmode, double massn, double gven, bool sbtoy, bool pseudo
      }
      ...
      for (uint ig = 0; ig < (uint) fNgveBins; ig++){
	double gven = fgveMin + ig*fgveStep; //TMath::Power(fgveMin*fgveMin*fgveMin*fgveMin + ig*fgveStep,0.25);
	double qmu = Qobs[im][ig] - Qobs[im][igmin]; // qmu
	fQmuProfile.at(im)->SetPoint(ig,gven,qmu);
	double pprimemu = (1. - PhiFun(TMath::Sqrt(qmu)))/PhiFun(TMath::Sqrt(Qobs_a[im][ig])-TMath::Sqrt(qmu)); // eq.11 of Frequentist_Limit_Recommendation.pdf
	fPprimeProfile.at(im)->SetPoint(ig,pprimemu,gven);
      }
      for (int k = 0; k<2; k++){ //90% CL, 95CL
	double mulim = fPprimeProfile.at(im)->Eval(0.10-0.05*k);
	double sigman = mulim/TMath::Sqrt(fQobs_a.at(im)->Eval(mulim)); // Eq 13. of the same
	double mupoints[5];
	for (int i=0; i<5; i++) mupoints[i] = sigman*asimLim[i][k]; // eq. 12., 14. of the same
	for (int i=0; i<2; i++){ // 1sigma, 2sigma
	  fAsimov_a[k].at(i)->SetPoint(im,massn,mupoints[2]);
	  fAsimov_a[k].at(i)->SetPointError(im,0.,0.,mupoints[2]-mupoints[1-i],mupoints[3+i]-mupoints[2]);
	}
      }
    }
    ...
    for (uint j=0; j<fNumberOfGenerations; j++){
      ...
      GenerateGeneralPseudoData(theta_B_i,false,0.,0.,true); // sbmode, mass, gve, toyoftoy

      double LB_Btoy;
      bool okfitb_btoy;
      nuisancePars thetabi = MaximizeL(theta_B_i,false,0.,0.,&LB_Btoy,&okfitb_btoy);

      // fill histograms

      fLB_BtoyHisto->Fill(LB_Btoy);
      if (okfitb_btoy) fillPullHistograms(false,0.,0.,false,false);//bool sbfitmode, double massn, double gven, bool sbtoy, bool pseudo

      for (uint im = 0; im < (uint) fNMassBins; im++){
	double massn = fMassMin + im*fMassStep;
	for (uint ig = 0; ig < (uint) fNgveBins; ig++){
	  double gven = fgveMin + ig*fgveStep; // TMath::Power(fgveMin*fgveMin*fgveMin*fgveMin + ig*fgveStep,0.25);
	  //	  cout << " begin to cycle " << im << " " << ig << " " << massn << " " << gven << endl;

	  double LSB_Btoy;
	  bool okfitsb_btoy;
	  nuisancePars thetasi = MaximizeL(theta_S_i[im][ig],true,massn,gven,&LSB_Btoy,&okfitsb_btoy);
	  //	  cout << " cont1 to cycle " << im << " " << ig << " " << massn << " " << gven << " " << okfitb_btoy << " "<< okfitsb_btoy<<endl;
	  if (okfitsb_btoy) fillPullHistograms(true,massn,gven,false,false); //bool sbfitmode, double massn, double gven, bool sbtoy, bool pseudo
	  
	  //	  cout << " cont2 to cycle " << im << " " << ig << " " << massn << " " << gven << " " << okfitb_btoy << " "<< okfitsb_btoy<<endl;
	  ...
	  if (okfitb_btoy && okfitsb_btoy) {
	    ...
	    //	    cout << " cont3 to cycle " << im << " " << ig << " " << massn << " " << gven << " " << okfitb_btoy << " "<< okfitsb_btoy<<endl;
	    // fill histograms
	    fLSB_BtoyHisto.at(im)->Fill(gven,LSB_Btoy);
	    fQbRelHisto.at(im)->Fill(gven,Qb-Qobs[im][ig]);
	    //	    cout << " done filling histograms " << im << " " << ig << " " << massn << " " << gven << endl;
	  }
	}
      }
    }
	...
	for (uint j=0; j< (uint) fNumberOfGenerations; j++){
	  GenerateGeneralPseudoData(theta_S_i[im][ig],true,massn,gven,true); // sbmode, mass, gve, toyoftoy

	  double LSB_SBtoy;
	  bool okfitsb_sbtoy;
	  nuisancePars thetasi = MaximizeL(theta_S_i[im][ig],true,massn,gven,&LSB_SBtoy,&okfitsb_sbtoy);
	  if (okfitsb_sbtoy) fillPullHistograms(true,massn,gven,true,false);// bool sbfitmode, double massn, double gven, bool sbtoy, bool pseudo

	  double LB_SBtoy;
	  bool okfitb_sbtoy;
	  nuisancePars thetabi = MaximizeL(theta_B_i,false,0.,0.,&LB_SBtoy,&okfitb_sbtoy);
	  if (okfitb_sbtoy) fillPullHistograms(false,massn,gven,true,false);// bool sbfitmode, double massn, double gven, bool sbtoy, bool pseudo
	...
	double CLs = Pb[im][ig]<1 ? Ps[im][ig]/(1-Pb[im][ig]): -1; // might be protected against 0/0
...
//NOTUsedvoid statisticalTreatmentTH::GenerateBackgroundPseudoData(nuisancePars nuis){
//NOTUsed  cout << " generate background only pseudoevent " << endl;
//NOTUsed  fObservables = fObservablesInit;
//NOTUsed  
//NOTUsed  // sample pots vs sqrts, potscaletrue, bkgtrue
//NOTUsed  double potscale = nuis.POTScaleTrue;
//NOTUsed  double bkgbiaspars[2] = {nuis.BkgBiasTrueP0,nuis.BkgBiasTrueP1};
//NOTUsed
//NOTUsed  if (fUseNuisance) {
//NOTUsed    if (fConfigPtr->GetCorrectBkgBias()){ // USE the bias curve, therefore remove the potscale parameter, substituted by the curve
//NOTUsed      potscale = 1;
//NOTUsed    // here sample the parameters from a bigauss
//NOTUsed      double diagpar[2] = { fRndmNumber->Gaus(0.,1),  fRndmNumber->Gaus(0.,1)};
//NOTUsed      for (int i=0; i<2; i++){
//NOTUsed	for (int j=0; j<2; j++){
//NOTUsed	  bkgbiaspars[i] += fInvCholeTransBkgBias[i][j]*diagpar[j];
//NOTUsed	}
//NOTUsed      }
//NOTUsed    } else {
//NOTUsed      potscale += fRndmNumber->Gaus(0.,fExpectedErrors.POTScaleErr);
//NOTUsed      bkgbiaspars[0] = 1;
//NOTUsed      bkgbiaspars[1] = 0;
//NOTUsed    }
//NOTUsed  }
//NOTUsed  // store values in fObservables
//NOTUsed  fObservables.POTScaleObs = potscale;
//NOTUsed  fObservables.BkgBiasObsP0 = bkgbiaspars[0];
//NOTUsed  fObservables.BkgBiasObsP1 = bkgbiaspars[1];
//NOTUsed//  cout << "Stored POT scales " << endl;
//NOTUsed
//NOTUsed  double bkgoverpotpars[2] = {nuis.BkgPerPOTVsSqrtsParTrue[0],nuis.BkgPerPOTVsSqrtsParTrue[1]};
//NOTUsed
//NOTUsed  // sample the B/POT vs sqrt(s) parameters
//NOTUsed
//NOTUsed  if (fConfigPtr->GetAssumeBkgOverPotCurve() && fUseNuisance){ 
//NOTUsed    double diagpar[2] = { fRndmNumber->Gaus(0.,1),  fRndmNumber->Gaus(0.,1)};
//NOTUsed    for (int i=0; i<2; i++){
//NOTUsed      for (int j=0; j<2; j++){
//NOTUsed	bkgoverpotpars[i] += fInvCholeTransBkgVsSqrts[i][j]*diagpar[j];
//NOTUsed      }
//NOTUsed    } 
//NOTUsed  } 
//NOTUsed
//NOTUsed  for (int i=0; i<2; i++) fObservables.BkgPerPOTVsSqrtsParObs[i] = bkgoverpotpars[i]; // store values in fObservables
//NOTUsed
//NOTUsed  // sample POT_i, Bkg_i and NObs
//NOTUsed
//NOTUsed  int npts = nuis.POTTrue.size();  
//NOTUsed  for (uint i=0; i<npts; i++){
//NOTUsed    double pottrue = nuis.POTTrue.at(i);
//NOTUsed    if (fUseNuisance) fObservables.POTObs.at(i) = pottrue + fRndmNumber->Gaus(0.,fExpectedErrors.POTLocalErr.at(i)); 
//NOTUsed    else  fObservables.POTObs.at(i) = pottrue;
//NOTUsed
//NOTUsed    double bkgtrue = 0;
//NOTUsed    if (fConfigPtr->GetAssumeBkgOverPotCurve()) {
//NOTUsed      bkgtrue = nuis.BkgPerPOTVsSqrtsParTrue[0] + nuis.BkgPerPOTVsSqrtsParTrue[1]*(fObservables.SqrtsObs.at(i)-SQRTSMID);
//NOTUsed    } else {
//NOTUsed      bkgtrue = nuis.BkgTrue.at(i);
//NOTUsed    }
//NOTUsed
//NOTUsed    if (fConfigPtr->GetAssumeBkgOverPotCurve()) {
//NOTUsed      fObservables.BkgObs.at(i) = bkgoverpotpars[0] + bkgoverpotpars[1]*(fObservables.SqrtsObs.at(i)-SQRTSMID);
//NOTUsed    } else {
//NOTUsed      if (fUseNuisance) fObservables.BkgObs.at(i)  = nuis.BkgTrue.at(i) + fRndmNumber->Gaus(0.,fExpectedErrors.BkgErr.at(i));
//NOTUsed      else              fObservables.BkgObs.at(i)  = nuis.BkgTrue.at(i);
//NOTUsed    }
//NOTUsed
//NOTUsed    double poismean = bkgtrue*pottrue*1E10; // true value of poisson mean
//NOTUsed    if (fConfigPtr->GetCorrectBkgBias()) {
//NOTUsed      poismean *= (nuis.BkgBiasTrueP0+nuis.BkgBiasTrueP1*(fObservables.SqrtsObs.at(i)-SQRTSMID));
//NOTUsed    } else {
//NOTUsed      poismean *= nuis.POTScaleTrue;
//NOTUsed    }
//NOTUsed    // store values in fObservables
//NOTUsed    if (fVerbosity >= 2){
//NOTUsed      cout << "storing values in fObservables point " << i << " / " << npts <<
//NOTUsed	" size of vectors " << fObservables.BkgObs.size() << " " << fObservables.POTObs.size() << " " << fObservables.NObs.size() << 
//NOTUsed	" size of Initvectors " << fObservablesInit.BkgObs.size() << " " << fObservablesInit.POTObs.size() << " " << fObservablesInit.NObs.size() << endl;
//NOTUsed    }
//NOTUsed    fObservables.NObs.at(i) = fRndmNumber->Gaus(poismean,TMath::Sqrt(poismean));
//NOTUsed    if (fVerbosity >= 2) cout << "stored values in fObservables point " << i << " / " << npts << endl;
//NOTUsed  }
//NOTUsed}

void statisticalTreatmentTH::GenerateGeneralPseudoData(nuisancePars nuis, bool sbmode, double massn, double gven, bool toyoftoy){  
  // To generate the pseudoevents:
  // - start from the nuisance "true" values and sample the Observed values of the nuisance parameters
  // - generate Nobs from the nuisance "true" values
  //
  // To generate the toy of toys:
  // - start from the nuisance "true" values, set the observed values of the nuisance parameters equal to the true values
  // - generate Nobs from the nuisance "true" values
  
  if (fVerbosity ==2) cout << " generate pseudoevent sbmode: " << sbmode << " toyoftoy = " << toyoftoy << endl;
  
  // sample pots vs sqrts, potscaletrue, bkgtrue
  double potscale = nuis.POTScaleTrue;
  double bkgbiaspars[2] = {nuis.BkgBiasTrueP0,nuis.BkgBiasTrueP1};
  ...
  // store values in fObservables if pseudoevents
  if ( !toyoftoy ) {
    fObservables.POTScaleObs = potscale;
    fObservables.BkgBiasObsP0 = bkgbiaspars[0];
    fObservables.BkgBiasObsP1 = bkgbiaspars[1];
  }
  if (fVerbosity ==2) cout << " done generating scale sbmode: " << sbmode << " toyoftoy = " << toyoftoy << endl;

  // sample the B/POT vs sqrt(s) parameters

  double bkgoverpotpars[3][2];
  for (int k=0; k<3; k++){ // scan periods
    for (int i=0; i<2; i++){ // scan periods
      bkgoverpotpars[k][i]= nuis.BkgPerPOTVsSqrtsParTrue[k][i];
    }
  }
  
  if (fUseNuisance && fConfigPtr->GetAssumeBkgOverPotCurve() && !toyoftoy){ 
    // here sample the parameters from a bigauss
    for (int k=0; k<3; k++){ // scan periods
      double diagpar[2] = { fRndmNumber->Gaus(0.,1),  fRndmNumber->Gaus(0.,1) };
      for (int i=0; i<2; i++){
	for (int j=0; j<2; j++){
	  if (k==0)      bkgoverpotpars[k][i] += fInvCholeTransBkgVsSqrtsScan1st[i][j]*diagpar[j];
	  else if (k==1) bkgoverpotpars[k][i] += fInvCholeTransBkgVsSqrtsScan2nd[i][j]*diagpar[j];
	  else           bkgoverpotpars[k][i] += fInvCholeTransBkgVsSqrtsScanAll[i][j]*diagpar[j];
	}
      }
    } 
  }  
  ...
  if ( !toyoftoy ) {
    for (int k=0; k<3; k++){
      for (int i=0; i<2; i++) fObservables.BkgPerPOTVsSqrtsParObs[k][i] = bkgoverpotpars[k][i];
    }
  }
  if (fVerbosity ==2) cout << " done generating bkg/pot parameters sbmode: " << sbmode << " toyoftoy = " << toyoftoy << endl;

  // pot scale correction
  double potslopecorr = fConfigPtr->GetUsePOTSlopeCorrection() ? nuis.POTSlopeCorrectionTrue : 0;
  if (fUseNuisance && fConfigPtr->GetUsePOTSlopeCorrection() && !toyoftoy){
    potslopecorr += fRndmNumber->Gaus(0.,1.)*fExpectedErrors.POTSlopeCorrectionErr;
  }
  // store values in fObservables
  if ( !toyoftoy ) fObservables.POTSlopeCorrectionObs = potslopecorr;
  ...
  if (fUseNuisance && fConfigPtr->GetAssumeEffiOverBkgCurve() && !toyoftoy){ // USE the effi curve, therefore do not treat the effi parameters
    // here sample the parameters from a bigauss
    for (int k=0; k<3; k++){ // scan periods
...
// write into fObservables
  ...
  if (fVerbosity ==2) cout << " done generating efficurve parameters sbmode: " << sbmode << " toyoftoy = " << toyoftoy << endl;
...
// write into fObservables
  ...
  if (fVerbosity ==2) cout << " done generating signal shape parameters sbmode: " << sbmode << " toyoftoy = " << toyoftoy << endl;

  // sample POT_i, Bkg_i, signal_i and NObs

  int npts = nuis.POTTrue.size();  
  for (uint i=0; i<npts; i++){
    // set true POT from nuisance
    double pottrue = nuis.POTTrue.at(i);
    double potobs  = nuis.POTTrue.at(i);
    // fluctuate POTObs only if UseNuisance && either straightfitmode = 1, 2 or you are making pseudo-events for expected limit
    if (fUseNuisance && (!toyoftoy || fConfigPtr->GetStraightFitMode())) potobs += fRndmNumber->Gaus(0.,fExpectedErrors.POTLocalErr.at(i));
    // store POTObs in the observables if either straightfitmode = 1, 2 or you are making pseudo-events for expected limit
    if (!toyoftoy || fConfigPtr->GetStraightFitMode()) fObservables.POTObs.at(i) = potobs;

    if (fVerbosity ==2) cout << " done fluctuating POTOBS " << i << endl;

    // set true BKG/POT from nuisance
    double bkgtrue = 0;
    if (fConfigPtr->GetAssumeBkgOverPotCurve()) {
      int iperiod = 2;
      if (fConfigPtr->GetAssumeBkgOverPotCurve() == 1) iperiod = fObservables.ScanPeriod.at(i);
      bkgtrue = nuis.BkgPerPOTVsSqrtsParTrue[iperiod][0] + nuis.BkgPerPOTVsSqrtsParTrue[iperiod][1]*(fObservables.SqrtsObs.at(i)-SQRTSMID);
    }
    else {
      bkgtrue = nuis.BkgTrue.at(i);
    }
    if (fVerbosity ==2) cout << " done setting BKGtrue/pot " << i << endl;
    double bkgobs = bkgtrue;

    // fluctuate BkgObs/POT: when straightfitmode = 2 it is done both for pseudoevents and toy-of-toy , otherwise only for pseudo-events
    if (fUseNuisance && (!toyoftoy || fConfigPtr->GetStraightFitMode() == 2)) {
      if (fConfigPtr->GetAssumeBkgOverPotCurve()) {
	int iperiod = 2;
	if (fConfigPtr->GetAssumeBkgOverPotCurve() == 1) iperiod = fObservables.ScanPeriod.at(i);
	bkgobs = bkgoverpotpars[iperiod][0] + bkgoverpotpars[iperiod][1]*(fObservables.SqrtsObs.at(i)-SQRTSMID); // use fluctuated parameters
      }
      else {
	bkgobs = bkgtrue + fRndmNumber->Gaus(0.,fExpectedErrors.BkgErr.at(i));
      }
    }
    if (!toyoftoy || fConfigPtr->GetStraightFitMode() == 2) fObservables.BkgObs.at(i)  = bkgobs;

//    if (!toyoftoy) {
//      if (fConfigPtr->GetAssumeBkgOverPotCurve()) { 
//	fObservables.BkgObs.at(i) = bkgoverpotpars[0] + bkgoverpotpars[1]*(fObservables.SqrtsObs.at(i)-SQRTSMID); // use fluctuated parameters
//      } else { // fluctuate bkg around their true value
//	if (fUseNuisance) fObservables.BkgObs.at(i)  = bkgtrue + fRndmNumber->Gaus(0.,fExpectedErrors.BkgErr.at(i));
//	else fObservables.BkgObs.at(i)  = bkgtrue;
//      }
//    }
    if (fVerbosity ==2) cout << " done generating bkgobs point " << i << " sbmode: " << sbmode << " toyoftoy = " << toyoftoy << endl;

    // true value of the bkg events
    double poismean = bkgtrue*pottrue*1E10; // true value of poisson mean
    double poismean_s = 0;

    if (sbmode) {
      // signal efficiency true value
      double effisigtrue = 0;
      if (fConfigPtr->GetAssumeEffiOverBkgCurve()){
	int iperiod = 2;
	if (fConfigPtr->GetAssumeEffiOverBkgCurve() == 1) iperiod = fObservables.ScanPeriod.at(i);
	effisigtrue = bkgtrue*(nuis.EffiOverBkgTrueP0[iperiod] + nuis.EffiOverBkgTrueP1[iperiod]*(fObservables.SqrtsObs.at(i)-SQRTSMID));
      } else {
    ...
    // for pseudoevents, fluctuate and set effi/B (always, even if sbmode = false!)
	...
	if (fUseNuisance) fObservables.SignalEffiLocalObs.at(i) = nuis.SignalEffiLocalTrue.at(i) + fRndmNumber->Gaus(0.,fExpectedErrors.SignalEffiLocalErr.at(i));
      ...
      if (fVerbosity ==2) cout << " evaluated effsig pnt " << i << " e= " << fObservables.SignalEffiLocalObs.at(i) << " sbmode: " << sbmode << " toyoftoy = " << toyoftoy << endl;
    }

    // store in fObservables fluctuated NObs
    
    double poismeantot = poismean + poismean_s;
    if (fConfigPtr->GetCorrectBkgBias()){
      poismeantot *= (nuis.BkgBiasTrueP0+nuis.BkgBiasTrueP1*(fObservables.SqrtsObs.at(i)-SQRTSMID));
    } else {
      poismeantot *= nuis.POTScaleTrue;
    }
    if (fVerbosity ==2) cout << "evaluated expected cnts pnt " << i << " S= " << poismean_s << " B = " << poismean << " tot = " << poismeantot << " sbmode: " << sbmode << " toyoftoy = " << toyoftoy << endl;
    
    // scale correction on the pot
    if (fConfigPtr->GetUsePOTSlopeCorrection()) poismeantot *= (1.+potslopecorr*fObservables.POTIntObs.at(i)/fObservables.POTIntObs.at(npts-1)); //scale corr

    fObservables.NObs.at(i) = fRndmNumber->Gaus(poismeantot,TMath::Sqrt(poismeantot));
    if (fVerbosity >= 2) cout << "stored values in fObservables point " << i << " / " << npts << endl;
  }
  if (fVerbosity ==2) cout << "done generate pseudoevent sbmode: " << sbmode << " toyoftoy = " << toyoftoy << endl;
}



//NOTUsedvoid statisticalTreatmentTH::GenerateSignalPlusBackgroundPseudoData(double massn, double gven, nuisancePars nuis){
//NOTUsed  fObservables = fObservablesInit;
//NOTUsed
//NOTUsed  // sample potscaletrue or, if needed, the parameters of the bkg bias curve: THOSE ARE SAMPLED ALWAYS, EVEN WHEN NO NUISANCES ARE CHOSEN
//NOTUsed
//NOTUsed  double potscale = nuis.POTScaleTrue;
//NOTUsed  double bkgbiaspars[2] = {0,0};
//NOTUsed  if (fConfigPtr->GetCorrectBkgBias()){ // USE the bias curve, therefore remove the potscale parameter, substituted by the curve
//NOTUsed    potscale = 1;
//NOTUsed    // here sample the parameters from a bigauss
//NOTUsed    double diagpar[2] = { fRndmNumber->Gaus(0.,1),  fRndmNumber->Gaus(0.,1)};
//NOTUsed    for (int i=0; i<2; i++){
//NOTUsed      bkgbiaspars[i] = 0;
//NOTUsed      for (int j=0; j<2; j++){
//NOTUsed	bkgbiaspars[i] += fInvCholeTransBkgBias[i][j]*diagpar[j];
//NOTUsed      }
//NOTUsed      if (i==0) bkgbiaspars[i] += nuis.BkgBiasTrueP0;
//NOTUsed      else      bkgbiaspars[i] += nuis.BkgBiasTrueP1;
//NOTUsed    }
//NOTUsed  } else {
//NOTUsed    potscale += fRndmNumber->Gaus(0.,fExpectedErrors.POTScaleErr);
//NOTUsed    bkgbiaspars[0] = 1;
...
//NOTUsed
//NOTUsed  // sample the B/POT vs sqrt(s) parameters
//NOTUsed  double bkgoverpotpars[2] = {0,0};
//NOTUsed  if (fConfigPtr->GetAssumeBkgOverPotCurve()){ 
//NOTUsed    if (fUseNuisance){
//NOTUsed      double diagpar[2] = { fRndmNumber->Gaus(0.,1),  fRndmNumber->Gaus(0.,1)};
//NOTUsed      for (int i=0; i<2; i++){
//NOTUsed	bkgoverpotpars[i] = 0;
//NOTUsed	for (int j=0; j<2; j++){
//NOTUsed	  bkgoverpotpars[i] += fInvCholeTransBkgVsSqrts[i][j]*diagpar[j];
//NOTUsed	}
//NOTUsed	bkgoverpotpars[i] += nuis.BkgPerPOTVsSqrtsParTrue[i];
//NOTUsed      }
//NOTUsed    }
//NOTUsed    else {
//NOTUsed      for (int i=0; i<2; i++) bkgoverpotpars[i] = nuis.BkgPerPOTVsSqrtsParTrue[i];
//NOTUsed    }
//NOTUsed  }  
//NOTUsed
//NOTUsed// store values in fObservables
//NOTUsed//  for (int i=0; i<2; i++) fObservables.BkgPerPOTVsSqrtsParObs[i] = bkgoverpotpars[i];
//NOTUsed
//NOTUsed  // if needed sample the parameters of the curves for the effi/(bkg/pot) vs sqrts
//NOTUsed  
//NOTUsed  double efficurvepars[3][2];
//NOTUsed  for (int k=0; k<3; k++){ // scan periods
//NOTUsed    for (int i=0; i<2; i++) efficurvepars[k][i] = 0;
//NOTUsed  }
//NOTUsed  if (fConfigPtr->GetAssumeEffiOverBkgCurve()){ // USE the effi curve, therefore do not treat the effi parameters
//NOTUsed    if (fUseNuisance){
//NOTUsed    // here sample the parameters from a bigauss
//NOTUsed      for (int k=0; k<3; k++){ // scan periods
//NOTUsed	double diagpar[3] = { fRndmNumber->Gaus(0.,1),  fRndmNumber->Gaus(0.,1),  fRndmNumber->Gaus(0.,1)};
//NOTUsed	for (int i=0; i<2; i++){
//NOTUsed	  efficurvepars[k][i] = 0;
//NOTUsed	  for (int j=0; j<2; j++){
//NOTUsed	    if (k==0)      efficurvepars[k][i] += fInvCholeTransEffiOverBkgScan1st[i][j]*diagpar[j];
//NOTUsed	    else if (k==1) efficurvepars[k][i] += fInvCholeTransEffiOverBkgScan2nd[i][j]*diagpar[j];
//NOTUsed	    else           efficurvepars[k][i] += fInvCholeTransEffiOverBkgScanAll[i][j]*diagpar[j];
//NOTUsed	  }
//NOTUsed	  if (i==0) efficurvepars[k][i] += nuis.EffiOverBkgTrueP0[k];
//NOTUsed	  else      efficurvepars[k][i] += nuis.EffiOverBkgTrueP1[k];
//NOTUsed	}
//NOTUsed      }
//NOTUsed    } else { // here do not use the nuisances -> keep the effioverbkg parameters frozen
//NOTUsed      for (int k=0; k<3; k++){
//NOTUsed	efficurvepars[k][0] = nuis.EffiOverBkgTrueP0[k];
//NOTUsed	efficurvepars[k][1] = nuis.EffiOverBkgTrueP1[k];
//NOTUsed      }
//NOTUsed    } 
//NOTUsed  }
//NOTUsed
//NOTUsed// write into fObservables
//NOTUsed//  for (int k=0; k<3; k++){
//NOTUsed//    fObservables.EffiOverBkgObsP0[k] = efficurvepars[k][0];
//NOTUsed//    fObservables.EffiOverBkgObsP1[k] = efficurvepars[k][1];
//NOTUsed//  }
//NOTUsed
//NOTUsed  // sample signal shape parameters
//NOTUsed  
//NOTUsed  double signalpeak = nuis.SignalPeakYieldTrue;
//NOTUsed  double bes = nuis.BESTrue;
//NOTUsed  double lorewidth = nuis.SignalLorentzianWidthTrue;
//NOTUsed  if (fUseNuisance){
//NOTUsed    signalpeak += fRndmNumber->Gaus(0.,fExpectedErrors.SignalPeakYieldErr);
//NOTUsed    bes += fRndmNumber->Gaus(0.,fExpectedErrors.BESErr);
//NOTUsed    lorewidth += fRndmNumber->Gaus(0.,fExpectedErrors.SignalLorentzianWidthErr);
//NOTUsed  }
//NOTUsed
//NOTUsed// write into fObservables
//NOTUsed//  fObservables.SignalPeakYieldObs = signalpeak;
//NOTUsed//  fObservables.BESObs = bes;
//NOTUsed//  fObservables.SignalLorentzianWidthObs = lorewidth;
//NOTUsed  
//NOTUsed  if (fVerbosity >= 2) {
//NOTUsed    std::cout << " generating s+b pseudo data for gven = " << gven << " mass = " << massn << " signalpeak= " << signalpeak << " bes = "<< bes << " lorewidth = " << lorewidth << std::endl;
//NOTUsed  }
//NOTUsed
//NOTUsed  int npts = nuis.POTTrue.size();
//NOTUsed  for (uint i=0; i<npts; i++){
//NOTUsed
//NOTUsed    double pottrue = nuis.POTTrue.at(i);
//NOTUsed    if (fUseNuisance)  pottrue += fRndmNumber->Gaus(0.,fExpectedErrors.POTLocalErr.at(i)); // if using assumeEffiOverBkg, it should be always 1
//NOTUsed
//NOTUsed    double bkgtrue = 0;
//NOTUsed    if (fConfigPtr->GetAssumeBkgOverPotCurve()) bkgtrue = bkgoverpotpars[0] + bkgoverpotpars[1]*(fObservables.SqrtsObs.at(i)-SQRTSMID);
//NOTUsed    else {
//NOTUsed      bkgtrue = nuis.BkgTrue.at(i);
//NOTUsed      if (fUseNuisance) bkgtrue += fRndmNumber->Gaus(0.,fExpectedErrors.BkgErr.at(i));
//NOTUsed    }
//NOTUsed
//NOTUsed    double effisig = 0;
//NOTUsed    if (fConfigPtr->GetAssumeEffiOverBkgCurve()){
//NOTUsed      int iperiod = 2;
//NOTUsed      if (fConfigPtr->GetAssumeEffiOverBkgCurve() == 1) iperiod = fObservables.ScanPeriod.at(i);
//NOTUsed      effisig = bkgtrue*(efficurvepars[iperiod][0] + efficurvepars[iperiod][1]*(fObservables.SqrtsObs.at(i)-SQRTSMID));
//NOTUsed    } else {
//NOTUsed      effisig = nuis.SignalEffiLocalTrue[i];
//NOTUsed      if (fUseNuisance) effisig += fRndmNumber->Gaus(0.,fExpectedErrors.SignalEffiLocalErr.at(i));
//NOTUsed    }
//NOTUsed    
//NOTUsed    double poismean_b = bkgtrue*pottrue*1E10;
//NOTUsed    if (fConfigPtr->GetCorrectBkgBias()){
//NOTUsed      poismean_b *= (bkgbiaspars[0]+bkgbiaspars[1]*(fObservables.SqrtsObs.at(i)-SQRTSMID));
//NOTUsed    } else {
//NOTUsed      poismean_b *= potscale;
//NOTUsed    }
//NOTUsed
//NOTUsed    double poismean_s = gven*gven*pottrue*1E10*effisig*Likelihood::SignalShape(signalpeak,bes,lorewidth,massn,fObservables.SqrtsObs.at(i));
//NOTUsed    if (fConfigPtr->GetCorrectBkgBias()){
//NOTUsed      poismean_s *= (bkgbiaspars[0]+bkgbiaspars[1]*(fObservables.SqrtsObs.at(i)-SQRTSMID));
//NOTUsed    } else {
//NOTUsed      poismean_s *= potscale;
//NOTUsed    }
//NOTUsed        
//NOTUsed    if (fVerbosity >= 2) {
//NOTUsed      std::cout << "point = " << i << " sqrts = " << fObservables.SqrtsObs.at(i) << " bkg= " << bkgtrue << " mu_bkg = " << poismean_b << " mu_sig= " << poismean_s << " pot = " << pottrue << std::endl;
//NOTUsed    }
//NOTUsed
//NOTUsed    // store values in fObservables
//NOTUsed    fObservables.BkgObs.at(i) = bkgtrue;
//NOTUsed    fObservables.POTObs.at(i) = pottrue;
//NOTUsed    fObservables.NObs.at(i) = fRndmNumber->Gaus(poismean_b+poismean_s,TMath::Sqrt(poismean_b+poismean_s));
//NOTUsed  }
//NOTUsed}
...
//NOTUsedvoid statisticalTreatmentTH::GenerateBackground(nuisancePars nuis){
//NOTUsed
//NOTUsed  if (fVerbosity >= 3) std::cout << "GenerateBackground...";
//NOTUsed  
//NOTUsed  // evaluate bkgmean from pots, potscaletrue OR the P0/P1 bias curve, bkgtrue
//NOTUsed  // do not re-init fObservables to fObservablesInit, only modify NObs and eventually (if straight fit mode = true) POT and BKG
//NOTUsed  int npts = nuis.POTTrue.size();
//NOTUsed
//NOTUsed  for (uint i=0; i<npts; i++){
//NOTUsed
//NOTUsed    double bkgtrue = 0;
//NOTUsed    if (fConfigPtr->GetAssumeBkgOverPotCurve()) bkgtrue = nuis.BkgPerPOTVsSqrtsParTrue[0] + nuis.BkgPerPOTVsSqrtsParTrue[1]*(fObservables.SqrtsObs.at(i)-SQRTSMID);
//NOTUsed    else bkgtrue = nuis.BkgTrue.at(i);
//NOTUsed
//NOTUsed    double pottrue = nuis.POTTrue.at(i);
//NOTUsed    if (fConfigPtr->GetStraightFitMode() == 1){
//NOTUsed      fObservables.POTObs.at(i) = pottrue + fRndmNumber->Gaus(0.,fExpectedErrors.POTLocalErr.at(i)); 
//NOTUsed    }
//NOTUsed
//NOTUsed    double poismean = bkgtrue*nuis.POTTrue.at(i)*1E10;
//NOTUsed    if (fConfigPtr->GetCorrectBkgBias()) poismean *= (nuis.BkgBiasTrueP0 + nuis.BkgBiasTrueP1*(fObservables.SqrtsObs.at(i)-SQRTSMID));
//NOTUsed    else                                 poismean *= nuis.POTScaleTrue;
//NOTUsed    // store values in fObservables
//NOTUsed//    fObservables.BkgObs.at(i) = bkgtrue;
//NOTUsed    fObservables.NObs.at(i) = fRndmNumber->Gaus(poismean,TMath::Sqrt(poismean));
//NOTUsed  }
//NOTUsed  if (fVerbosity >= 3) std::cout << "...done" << std::endl;
//NOTUsed}

//NOTUsedvoid statisticalTreatmentTH::GenerateSignalPlusBackground(double massn, double gven, nuisancePars nuis){
//NOTUsed  if (fVerbosity >= 3) std::cout << "GenerateSignalPlusBackground...";
//NOTUsed  // sample pots vs sqrts, potscaletrue, bkgtrue
//NOTUsed  // do not re-init fObservables to fObservablesInit, only modify NObs and eventually (if straight fit mode = 1) POT
//NOTUsed
//NOTUsed  int npts = nuis.POTTrue.size();
//NOTUsed  for (uint i=0; i<npts; i++){
//NOTUsed    double bkgtrue = 0;
...
//NOTUsed
//NOTUsed    double poismean_b = bkgtrue*pottrue*1E10;
//NOTUsed    if (fConfigPtr->GetCorrectBkgBias()) poismean_b *= (nuis.BkgBiasTrueP0 + nuis.BkgBiasTrueP1*(fObservables.SqrtsObs.at(i)-SQRTSMID));
//NOTUsed    else                                 poismean_b *= nuis.POTScaleTrue;
//NOTUsed
//NOTUsed//    double eRes = (massn*massn - 2*me*me)/(2.*(me+Wb));
//NOTUsed//    double eBeam = (fObservables.SqrtsObs.at(i)*fObservables.SqrtsObs.at(i) - 2*me*me)/(2.*me); // s = 2me^2 + 2meEbeam -> eBeam = (s-2me^2) / 2me
//NOTUsed
//NOTUsed    double poismean_s = gven*gven*pottrue*1E10*
//NOTUsed      Likelihood::SignalShape(nuis.SignalPeakYieldTrue,nuis.BESTrue,nuis.SignalLorentzianWidthTrue,massn,fObservables.SqrtsObs.at(i));
//NOTUsed
//NOTUsed    if (fConfigPtr->GetCorrectBkgBias()) poismean_s *= (nuis.BkgBiasTrueP0 + nuis.BkgBiasTrueP1*(fObservables.SqrtsObs.at(i)-SQRTSMID));
//NOTUsed    else                                 poismean_s *= nuis.POTScaleTrue;
//NOTUsed
//NOTUsed    if (fConfigPtr->GetAssumeEffiOverBkgCurve()) {
//NOTUsed      int iperiod = 2;
//NOTUsed      if (fConfigPtr->GetAssumeEffiOverBkgCurve() == 1) iperiod = fObservables.ScanPeriod.at(i);
//NOTUsed      
//NOTUsed      poismean_s *= bkgtrue*(
//NOTUsed			     nuis.EffiOverBkgTrueP0[iperiod] +
//NOTUsed			     nuis.EffiOverBkgTrueP1[iperiod]*(fObservables.SqrtsObs.at(i)-SQRTSMID));
//NOTUsed    }
//NOTUsed    else {
//NOTUsed      poismean_s *= nuis.SignalEffiLocalTrue[i];
//NOTUsed    }
//NOTUsed    //    double poismean_s = nuis.SignalPeakYieldTrue*gven*gven*nuis.POTTrue.at(i)*1E10*nuis.POTScaleTrue*nuis.SignalEffiLocalTrue[i]*TMath::Voigt(eBeam-eRes,nuis.BESTrue*eBeam,nuis.SignalLorentzianWidthTrue*2,4);
//NOTUsed
//NOTUsed//    double argtight = (fObservables.SqrtsObs.at(i)-massn)*(fObservables.SqrtsObs.at(i)-massn)/(2.*nuis.SignalSigmaHiTrue*nuis.SignalSigmaHiTrue);
//NOTUsed//    double argloose = (fObservables.SqrtsObs.at(i)-massn)*(fObservables.SqrtsObs.at(i)-massn)/(2.*nuis.SignalSigmaLoTrue*nuis.SignalSigmaLoTrue);
//NOTUsed//    double poismean_s = fObservables.SignalPeakYieldObs*gven*gven*nuis.POTTrue.at(i)*nuis.POTScaleTrue*nuis.SignalEffiLocalTrue[i]*(
//NOTUsed//												      TMath::Exp(-argtight)*nuis.SignalRatioTrue + 
//NOTUsed//												      TMath::Exp(-argloose)*(1-nuis.SignalSigmaLoTrue)
//NOTUsed//												      ); 
//NOTUsed  
//NOTUsed    // store values in fObservables
//NOTUsed//    fObservables.BkgObs.at(i) = bkgtrue;
...
//NOT useddouble statisticalTreatmentTH::EvaluateLSBGivenNObs(double mass, double gve, nuisancePars nuis, bool* fitok){ // given M,eps evaluate Lsb with frozen nuisance pars
//NOT used  if (fVerbosity >= 3) std::cout << "EvaluateLSBGivenNObs " << mass << " " << gve << "...";
//NOT used  int npts = fObservables.SqrtsObs.size();
//NOT used  fFitterSB.Config().ParSettings(0).SetValue(0); // S+B fit
//NOT used  fFitterSB.Config().ParSettings(1).SetValue(mass); 
//NOT used  fFitterSB.Config().ParSettings(2).SetValue(gve); 
//NOT used
//NOT used  if (fConfigPtr->GetCorrectBkgBias()){ // USE the bias curve, therefore remove the potscale parameter, substituted by the curve    
//NOT used    fFitterSB.Config().ParSettings(8+3*npts).SetValue(nuis.BkgBiasTrueP0); // Set P0 of the bias curve
//NOT used    fFitterSB.Config().ParSettings(9+3*npts).SetValue(nuis.BkgBiasTrueP1); // Set P1 of the bias curve    
//NOT used  } else { // DO NOT USE the pol1 bias curve
//NOT used    fFitterSB.Config().ParSettings(6+2*npts).SetValue(nuis.POTScaleTrue);  // set the potscale
//NOT used  }
//NOT used  
//NOT used  fFitterSB.Config().ParSettings(3+2*npts).SetValue(nuis.SignalPeakYieldTrue);       // set the 3 signal shape parameters
//NOT used  fFitterSB.Config().ParSettings(4+2*npts).SetValue(nuis.SignalLorentzianWidthTrue); // set the 3 signal shape parameters
//NOT used  fFitterSB.Config().ParSettings(5+2*npts).SetValue(nuis.BESTrue);                   // set the 3 signal shape parameters
//NOT used  if (fConfigPtr->GetAssumeEffiOverBkgCurve()){
//NOT used    int kmin = 2;
//NOT used    int kmax = 3;
//NOT used    if (fConfigPtr->GetAssumeEffiOverBkgCurve() == 1){
//NOT used      kmin = 0;
//NOT used      kmax = 2;      
//NOT used    }
//NOT used    for (int k= kmin; k<kmax; k++){
//NOT used      fFitterSB.Config().ParSettings(11+3*npts+2*k).SetValue(nuis.EffiOverBkgTrueP0[k]);
//NOT used      fFitterSB.Config().ParSettings(12+3*npts+2*k).SetValue(nuis.EffiOverBkgTrueP1[k]);
//NOT used    }
//NOT used  }
//NOT used  else {
//NOT used    // set single-point efficiencies
//NOT used    for (int i=0; i<npts; i++) fFitterSB.Config().ParSettings(3*npts+i).SetValue(nuis.SignalEffiLocalTrue.at(i));    
//NOT used  }
//NOT used
//NOT used  // set pot and bkg (not always needed)
//NOT used
//NOT used  if (fConfigPtr->GetAssumeBkgOverPotCurve()) {
//NOT used    for (uint i=0; i<2; i++) fFitterSB.Config().ParSettings(19+3*npts+i).SetValue(nuis.BkgPerPOTVsSqrtsParTrue[i]);
//NOT used  }
//NOT used  else {
//NOT used    for (uint i=0; i<(uint)npts; i++)            fFitterSB.Config().ParSettings(7+2*npts+i).SetValue(nuis.BkgTrue.at(i)); // fix B point-by-point
//NOT used  }
//NOT used
//NOT used  for (uint i=0; i<(uint)npts; i++)              fFitterSB.Config().ParSettings(3+i).SetValue(nuis.POTTrue.at(i)); // fix POT point-by-point
//NOT used
//NOT used
//NOT used  fLikeli.SetObservables(fObservables);
//NOT used
//NOT used  streambuf *old = cout.rdbuf(); // <-- save        
//NOT used  stringstream ss;  
//NOT used  cout.rdbuf (ss.rdbuf());       // <-- redirect
//NOT used  fFitterSB.EvalFCN();
//NOT used  const ROOT::Fit::FitResult & result = fFitterSB.Result();    
//NOT used  double likelieval = result.MinFcnValue();
//NOT used  cout.rdbuf (old);              // <-- restore
//NOT used  *fitok = fFitterSB.FitFCN();
//NOT used  double outputlikeli=0;
//NOT used  if (*fitok) {
//NOT used    outputlikeli = fFitterSB.Result().MinFcnValue();
//NOT used  }
//NOT used  else {
//NOT used    outputlikeli = likelieval;
//NOT used  }
//NOT used  if (fVerbosity >= 3) std::cout << " outputlikeli = " << outputlikeli << "...done" << std::endl;
//NOT used  return outputlikeli;
//NOT used}

nuisancePars statisticalTreatmentTH::MaximizeL(nuisancePars nuis, bool sbmode, double mass, double gve, double* lsb, bool* fitstatus){ // minimize L. Returns fit nuisance pars, Lmin, fitstatus
  if (fVerbosity >= 3) std::cout << "MaximizeL..." << sbmode << " " << mass << " " << gve << endl;
  ...
  ROOT::Fit::Fitter* fitter;
  fitter = &fFitterB;
  if (sbmode) fitter = &fFitterSB;

  if (sbmode){
    fitter->Config().ParSettings(1).SetValue(mass); 
    fitter->Config().ParSettings(2).SetValue(gve); 
  }

  if (fConfigPtr->GetCorrectBkgBias()){ // USE the bias curve, therefore remove the potscale parameter, substituted by the curve    
    fitter->Config().ParSettings(8+3*npts).SetValue(nuis.BkgBiasTrueP0); // Set P0 of the bias curve
    fitter->Config().ParSettings(9+3*npts).SetValue(nuis.BkgBiasTrueP1); // Set P1 of the bias curve    
  } else { // DO NOT USE the pol1 bias curve
    fitter->Config().ParSettings(6+2*npts).SetValue(nuis.POTScaleTrue);  // set the potscale
  }

  if (fConfigPtr->GetAssumeBkgOverPotCurve() == 1) {
    for (uint j=0; j<2; j++)   
      for (uint i=0; i<2; i++)   fitter->Config().ParSettings(19+3*npts+i+2*j).SetValue(nuis.BkgPerPOTVsSqrtsParTrue[j][i]); // set B parameters
  } else if (fConfigPtr->GetAssumeBkgOverPotCurve() == 2) {
    for (uint i=0; i<2; i++)   fitter->Config().ParSettings(19+3*npts+i+4).SetValue(nuis.BkgPerPOTVsSqrtsParTrue[2][i]); // set B parameters
  } else {
    if (fConfigPtr->GetStraightFitMode() != 2){
      for (int i=0; i<npts; i++){
	fitter->Config().ParSettings(7+2*npts+i).SetValue(nuis.BkgTrue.at(i)); // set B point-by-point
      }
    }
  }

  // set pot and bkg if needed
  if (fUseNuisance && (fConfigPtr->GetStraightFitMode() != 1 && fConfigPtr->GetStraightFitMode() != 2)){
    for (uint i=0; i<(uint)npts; i++)              fitter->Config().ParSettings(3+i).SetValue(nuis.POTTrue.at(i)); // set POT point-by-point
  }

  if (fConfigPtr->GetUsePOTSlopeCorrection()){ // USE correction slope vs integrated pot
    fitter->Config().ParSettings(26+3*npts).SetValue(nuis.POTSlopeCorrectionTrue);
  }

  // for the signal+bkg fit, set the other parameters

  if (sbmode){
    fitter->Config().ParSettings(3+2*npts).SetValue(nuis.SignalPeakYieldTrue);       // set the 3 signal shape parameters
    fitter->Config().ParSettings(4+2*npts).SetValue(nuis.SignalLorentzianWidthTrue); // set the 3 signal shape parameters
    fitter->Config().ParSettings(5+2*npts).SetValue(nuis.BESTrue);                   // set the 3 signal shape parameters
    if (fConfigPtr->GetAssumeEffiOverBkgCurve()){
      int kmin = 2;
      int kmax = 3;
	...
	fitter->Config().ParSettings(11+3*npts+2*k).SetValue(nuis.EffiOverBkgTrueP0[k]);
	fitter->Config().ParSettings(12+3*npts+2*k).SetValue(nuis.EffiOverBkgTrueP1[k]);
      }
    }
    else {
      // set single-point efficiencies
      for (int i=0; i<npts; i++) fitter->Config().ParSettings(3*npts+i).SetValue(nuis.SignalEffiLocalTrue.at(i));    
    }
  }

  if (fVerbosity >= 3){    
    for (int i=0; i<fitter->Config().NPar(); i++){
      std::cout << "Fit parameter " << i << " set at " << fitter->Config().ParamsValues().at(i) << " isFixed? " << fitter->Config().ParSettings(i).IsFixed() << std::endl;
    }
  }

  fLikeli.SetObservables(fObservables);

  int npars = 27+3*npts; // FRAGILE 
  int freePars = 0;
  for (uint ip=0; ip < npars; ip++) {
    if (!fitter->Config().ParSettings(ip).IsFixed()) freePars++;
  }
  if (fVerbosity >=2) {
    std::cout << "EvaluateL: about to evaluate the likelihood " << freePars << " with sbmode = " << sbmode << endl;
  }
  
  // first, evaluate the likelihood
  streambuf *old = cout.rdbuf(); // <-- save        
  stringstream ss;  
  cout.rdbuf (ss.rdbuf());       // <-- redirect
  fitter->EvalFCN();
  const ROOT::Fit::FitResult & result = fitter->Result();    
  double likelieval = result.MinFcnValue(); // evaluation of the likelihood without fitting
  cout.rdbuf (old);              // <-- restore

  // Fit the FCN
  
  bool okfit = fitter->FitFCN();
  double outputlikeli=0;
  if (!okfit) { // try recovery

    if (fVerbosity >= 2){
      std::cout << "Likelihood Fit failed sbmode " << sbmode << " mass,coupling = " << mass << " , " << gve << std::endl;
      const ROOT::Fit::FitResult & result = fitter->Result();    
      cout << "Fit status " << result.Status() << " isValid " << result.IsValid() << endl;
      result.Print(std::cout);
    }

    // do a new fit after re-setting the fitter

    initFitters(!sbmode, sbmode? fTheta_S : fTheta_B); // re-init with the original nuisance parameters    
    if (sbmode){
      fitter->Config().ParSettings(1).SetValue(mass); 
      fitter->Config().ParSettings(2).SetValue(gve);
    }

    okfit = fitter->FitFCN(); //re-fit
    if (okfit) {

      if (fVerbosity >= 0) std::cout << "Likelihood fit failed but recovered " << sbmode << std::endl;
      const ROOT::Fit::FitResult & result = fitter->Result();    
      if (fVerbosity >= 0) {
	cout << "Fit status " << result.Status() << " isValid " << result.IsValid() << endl;
	result.Print(std::cout);
      }
      outputlikeli = result.MinFcnValue();
    } else {
      if (fVerbosity >=0) std::cout << "Likelihood Fit failed and failed " << sbmode << std::endl;
    }
  ...
  if (!okfit) {
    ...
    *lsb = -1;
    ...
    if (fConfigPtr->GetAssumeBkgOverPotCurve() == 1) {
      for (uint j=0; j<2; j++) {
	for (uint i=0; i<2; i++) fitNuisance.BkgPerPOTVsSqrtsParTrue[j][i] = parRes[19+3*npts+i+2*j];
      }	
    } else if (fConfigPtr->GetAssumeBkgOverPotCurve() == 2) {
	for (uint i=0; i<2; i++) fitNuisance.BkgPerPOTVsSqrtsParTrue[2][i] = parRes[19+3*npts+i+4];      
    } else {
      for (int i=0; i<npts; i++) fitNuisance.BkgTrue.push_back(parRes[7+2*npts+i]);      
    }
    // store slope correction vs pot int
    if (fConfigPtr->GetUsePOTSlopeCorrection()){
      fitNuisance.POTSlopeCorrectionTrue = parRes[26+3*npts];
    }
...
//NOT usednuisancePars statisticalTreatmentTH::MaximizeLSBGivenNObs(double mass, double gve, double* lsb){ // given M,eps maximize Lsb wrt nuisance pars, return "profiled" nuisance pars
//NOT used  if (fVerbosity >= 3) std::cout << "MaximizeLSBGivenNObs..." << mass << " " << gve;
//NOT used
//NOT used  int npts = fObservables.SqrtsObs.size();
//NOT used  nuisancePars fitNuisance;
//NOT used  
//NOT used  fFitterSB.Config().ParSettings(0).SetValue(0); // S+B fit
//NOT used  fFitterSB.Config().ParSettings(1).SetValue(mass); 
//NOT used  fFitterSB.Config().ParSettings(2).SetValue(gve); 
//NOT used
//NOT used  if (fVerbosity >= 3){
//NOT used    for (int i=0; i<fFitterSB.Config().NPar(); i++){
//NOT used      std::cout << "S+B Fit parameter " << i << " set at " << fFitterSB.Config().ParamsValues().at(i) << " isFixed? " << fFitterSB.Config().ParSettings(i).IsFixed() << std::endl;
//NOT used    }
//NOT used  }
//NOT used
//NOT used  fLikeli.SetObservables(fObservables);
//NOT used  bool okfit = fFitterSB.FitFCN();
//NOT used  if (!okfit) {
//NOT used    if (fVerbosity >= 2){
//NOT used      std::cout << "LSB Fit failed" << std::endl;
//NOT used      const ROOT::Fit::FitResult & result = fFitterSB.Result();    
//NOT used      cout << "Fit status " << result.Status() << " isValid " << result.IsValid() << endl;
//NOT used      result.Print(std::cout);
//NOT used    }
//NOT used    initFitters(kFALSE);
//NOT used    fFitterSB.Config().ParSettings(0).SetValue(0); // S+B fit
//NOT used    fFitterSB.Config().ParSettings(1).SetValue(mass); 
//NOT used    fFitterSB.Config().ParSettings(2).SetValue(gve); 
//NOT used    okfit = fFitterSB.FitFCN();
//NOT used    if (fVerbosity >= 0){
//NOT used      if (okfit) {
//NOT used	std::cout << "LSB Fit failed but recovered " << std::endl;
//NOT used	const ROOT::Fit::FitResult & result = fFitterSB.Result();    
//NOT used	cout << "Fit status " << result.Status() << " isValid " << result.IsValid() << endl;
//NOT used	result.Print(std::cout);
//NOT used      }
//NOT used      else       std::cout << "LSB Fit failed and failed " << std::endl;
//NOT used    }
//NOT used    if (!okfit && fVerbosity >= 2){
//NOT used      const ROOT::Fit::FitResult & result = fFitterSB.Result();    
//NOT used      cout << "Fit status " << result.Status() << " isValid " << result.IsValid() << endl;
//NOT used      result.Print(std::cout);
//NOT used    }
//NOT used  }
//NOT used  if (!okfit) {
//NOT used    fitNuisance.isNotFilled = true;
//NOT used    *lsb = -1;
//NOT used  }
//NOT used  else {
//NOT used    const ROOT::Fit::FitResult & result = fFitterSB.Result();    
//NOT used    if (fVerbosity >= 2) cout << "Fit status " << result.Status() << " isValid " << result.IsValid() << endl;
//NOT used    if (fVerbosity == 3) {
//NOT used      result.Print(std::cout);
//NOT used      std::cout << "Total params " << result.NTotalParameters() << " free params " << result.NFreeParameters() << " Chi2 " << result.Chi2() << " Edm " << result.Edm() << " minFCNvalue " << result.MinFcnValue() << " minimizer type " << result.MinimizerType() << " Ncalls " << result.NCalls() << " Ndf " << result.Ndf() << " prob " << result.Prob() << std::endl;
//NOT used    }
//NOT used    // store output fit results into the nuisance parameters
//NOT used    const double* parRes = result.GetParams();    
//NOT used    // stopre pot values
//NOT used    for (int i=0; i<npts; i++) fitNuisance.POTTrue.push_back(parRes[3+i]);      
//NOT used    
//NOT used    fitNuisance.SignalPeakYieldTrue = parRes[3+2*npts];
//NOT used    fitNuisance.SignalLorentzianWidthTrue = parRes[4+2*npts];
//NOT used    fitNuisance.BESTrue           = parRes[5+2*npts];
//NOT used
//NOT used    // store potscale values
//NOT used    if (fConfigPtr->GetCorrectBkgBias()){ // USE the bias curve, therefore remove the potscale parameter, substituted by the curve
//NOT used      fitNuisance.BkgBiasTrueP0 = parRes[8+3*npts]; // p0 bias curve result
//NOT used      fitNuisance.BkgBiasTrueP1 = parRes[9+3*npts]; // p1 bias curve result
//NOT used    } else { // DO NOT USE the bias curve, fit the pot scale
//NOT used      fitNuisance.POTScaleTrue      = parRes[6+2*npts]; // pot scale result
//NOT used    }
//NOT used
//NOT used    // store bkg true values
//NOT used    if (fConfigPtr->GetAssumeBkgOverPotCurve()) {
//NOT used      for (uint i=0; i<2; i++) fitNuisance.BkgPerPOTVsSqrtsParTrue[i] = parRes[19+3*npts+i];
//NOT used    } else {
//NOT used      for (int i=0; i<npts; i++) fitNuisance.BkgTrue.push_back(parRes[7+2*npts+i]);      
//NOT used    }
//NOT used
//NOT used    // store efficiency values
//NOT used    if (fConfigPtr->GetAssumeEffiOverBkgCurve()){ // USE the effi/(bkg/pot) curve vs sqrt(s) 
//NOT used      for (int i=0; i<3; i++){
//NOT used	fitNuisance.EffiOverBkgTrueP0[i] = parRes[11+3*npts+2*i]; // 11+3*npts, 13+3*npts, 15+3*npts
//NOT used	fitNuisance.EffiOverBkgTrueP1[i] = parRes[12+3*npts+2*i]; // 12+3*npts, 14+3*npts, 16+3*npts
//NOT used      }
//NOT used    } else { // DO NOT USE the effi/(bkg/pot) curve --> fit the efficiencies point by point
//NOT used      for (int i=0; i<npts; i++) fitNuisance.SignalEffiLocalTrue.push_back(parRes[3+npts+i]);      
//NOT used    }
//NOT used
//NOT used    // flag nuisance as filled
//NOT used    fitNuisance.isNotFilled = false;
//NOT used    
//NOT used    // store the minimum lsb value [chi^2 like]
//NOT used    *lsb = result.MinFcnValue();
//NOT used  }
//NOT used  if (fVerbosity >= 3) std::cout << " outputlikeli = " << *lsb << " ...done" << std::endl;
//NOT used  return fitNuisance;
//NOT used}

//NOT useddouble statisticalTreatmentTH::EvaluateLBGivenNObs(nuisancePars nuis, bool* fitok){ // evaluate Lb with frozen nuisance pars
//NOT used  if (fVerbosity >= 3) std::cout << "EvaluateLBGivenNObs...";
//NOT used  int npts = fObservables.SqrtsObs.size();
//NOT used  fFitterB.Config().ParSettings(0).SetValue(1); // B fit  
//NOT used
//NOT used  if (fConfigPtr->GetCorrectBkgBias()){ // USE the bias curve, therefore remove the potscale parameter, substituted by the curve    
//NOT used    fFitterB.Config().ParSettings(8+3*npts).SetValue(nuis.BkgBiasTrueP0); // Set P0 of the bias curve
//NOT used    fFitterB.Config().ParSettings(9+3*npts).SetValue(nuis.BkgBiasTrueP1); // Set P1 of the bias curve    
//NOT used  } else { // DO NOT USE the pol1 bias curve
//NOT used    fFitterB.Config().ParSettings(6+2*npts).SetValue(nuis.POTScaleTrue);  // set the potscale
//NOT used  }
//NOT used
//NOT used  if (fConfigPtr->GetAssumeBkgOverPotCurve()) {
//NOT used    for (uint i=0; i<2; i++)   fFitterB.Config().ParSettings(19+3*npts+i).SetValue(nuis.BkgPerPOTVsSqrtsParTrue[i]); // fix B parameters
//NOT used  } else {
//NOT used    for (int i=0; i<npts; i++){
//NOT used      fFitterB.Config().ParSettings(7+2*npts+i).SetValue(nuis.BkgTrue.at(i)); // fix B point-by-point
//NOT used    }
//NOT used  }
//NOT used
//NOT used  // set pot and bkg if needed
//NOT used  if (fUseNuisance && fConfigPtr->GetStraightFitMode() != 1){
//NOT used    for (uint i=0; i<(uint)npts; i++)              fFitterB.Config().ParSettings(3+i).SetValue(nuis.POTTrue.at(i)); // fix POT point-by-point
//NOT used  }
//NOT used
//NOT used  fLikeli.SetObservables(fObservables);
//NOT used//  const ROOT::Fit::FitResult & result = fFitterB.Result();    
//NOT used//  return result.MinFcnValue();
//NOT used
//NOT used  streambuf *old = cout.rdbuf(); // <-- save        
//NOT used  stringstream ss;  
//NOT used  cout.rdbuf (ss.rdbuf());       // <-- redirect
//NOT used
//NOT used  fFitterB.EvalFCN();
//NOT used  const ROOT::Fit::FitResult & result = fFitterB.Result();  
//NOT used  double likelieval = result.MinFcnValue();
//NOT used  cout.rdbuf (old);              // <-- restore
//NOT used
//NOT used  *fitok = fFitterB.FitFCN();
//NOT used  double likeliout = 0;
//NOT used  if (*fitok) {
//NOT used    likeliout = fFitterB.Result().MinFcnValue();
//NOT used  }
//NOT used  else {
//NOT used    likeliout = likelieval;
//NOT used  }
//NOT used  if (fVerbosity >= 3) std::cout << " outputlikeli = " << likeliout << " ...done" << std::endl;
//NOT used  return likeliout;
//NOT used}

//NOT usednuisancePars statisticalTreatmentTH::MaximizeLBGivenNObs(double* lb){ // maximize Lb wrt nuisance pars, return "profiled" nuisance pars
//NOT used  if (fVerbosity >= 3) std::cout << "MaximizeLBGivenNobs...";
//NOT used
//NOT used  int npts = fObservables.SqrtsObs.size();
//NOT used  nuisancePars fitNuisance;
//NOT used  
...
//NOT used  fLikeli.SetObservables(fObservables);
//NOT used  
//NOT used  bool okfit = fFitterB.FitFCN();
//NOT used  if (!okfit) {
//NOT used    std::cerr << "LB Fit failed" << std::endl;
//NOT used    fitNuisance.isNotFilled = true;
//NOT used    *lb = -1;
//NOT used  }
//NOT used  else {
//NOT used    const ROOT::Fit::FitResult & result = fFitterB.Result();    
//NOT used    if (fVerbosity == 3) result.Print(std::cout);
//NOT used
//NOT used    // store output fit results into the nuisance parameters
//NOT used    const double* parRes = result.GetParams();    
//NOT used    for (int i=0; i<npts; i++){
//NOT used      fitNuisance.POTTrue.push_back(parRes[3+i]);
//NOT used    }
//NOT used    if (fConfigPtr->GetCorrectBkgBias()){ // USE the bias curve, therefore remove the potscale parameter, substituted by the curve
//NOT used      fitNuisance.BkgBiasTrueP0 = parRes[8+3*npts]; // p0 bias curve result
//NOT used      fitNuisance.BkgBiasTrueP1 = parRes[9+3*npts]; // p1 bias curve result
...
//NOT used    } else {
//NOT used      for (int i=0; i<npts; i++){
//NOT used	fitNuisance.BkgTrue.push_back(parRes[7+2*npts+i]);
//NOT used      }
//NOT used    }
//NOT used
//NOT used    fitNuisance.isNotFilled = false;
//NOT used    
//NOT used    // store the minimum lsb value [chi^2 like]
//NOT used    *lb = result.MinFcnValue();
//NOT used  }
//NOT used  if (fVerbosity >= 3) std::cout << " outputlikeli = " << *lb << " ...done" << std::endl;
//NOT used  return fitNuisance;
//NOT used}
...
void statisticalTreatmentTH::SimulatePseudoDataToFile(TString filename){
  ...
  for (int j=0; j<fNumberOfGenerationsExpectedLimit; j++){
    ...
    otherParsGraphBkgOnly[j] = new TGraph(19); otherParsGraphBkgOnly[j]->SetName(Form("gOther_%d",j));
    for (uint i=0; i<fObservables.SqrtsObs.size(); i++){
  ...
  for (uint im = 0; im < (uint) fNMassBins; im++){
	...
	otherParsGraph[j] = new TGraph(19); otherParsGraph[j]->SetName(Form("gOther_Mass_%f_gve_%f_%d",massn,gven,j));
...
void statisticalTreatmentTH::SetObservablesAsimov(nuisancePars nuis){
  ...
  fObservables.SignalEffiLocalObs = nuis.SignalEffiLocalTrue; // estimated efficiency 
  fObservables.POTScaleObs = nuis.POTScaleTrue;             // estimated value of the POT scale
  fObservables.BkgObs = nuis.BkgTrue;                       // estimated value of the ratio Nbkg/POT per point
  fObservables.SignalPeakYieldObs = nuis.SignalPeakYieldTrue; // yield at the resonance peak
  fObservables.SignalLorentzianWidthObs = nuis.SignalLorentzianWidthTrue; // lorentzian width of the signal shape [in terms of pbeam]
  fObservables.BESObs = nuis.BESTrue;  // relative beam energy spread 

  fObservables.BkgBiasObsP0 = nuis.BkgBiasTrueP0;
  fObservables.BkgBiasObsP1 = nuis.BkgBiasTrueP1;
  for (int i=0; i<3; i++){
    fObservables.EffiOverBkgObsP0[i] = nuis.EffiOverBkgTrueP0[i];
    fObservables.EffiOverBkgObsP1[i] = nuis.EffiOverBkgTrueP1[i];
  }  
  for (int q=0; q<3; q++){
    for (int i=0; i<2; i++){
      fObservables.BkgPerPOTVsSqrtsParObs[q][i] = nuis.BkgPerPOTVsSqrtsParTrue[q][i];
    }
  }
  int npts = fObservables.SqrtsObs.size();
...
void statisticalTreatmentTH::SetObservablesFromFile(TString filename, Bool_t sbmode, Double_t mass, Double_t gve, Int_t count){
  ...
  TGraph* nobsGraph = (TGraph*) filo->Get(Form("%s/gNObs_%s",dirname.Data(),graphname.Data())); 
  TGraph* nbkgGraph = (TGraph*) filo->Get(Form("%s/gNBkg_%s",dirname.Data(),graphname.Data())); 
  TGraph* npotGraph = (TGraph*) filo->Get(Form("%s/gNPOT_%s",dirname.Data(),graphname.Data())); 
  TGraph* effiGraph = (TGraph*) filo->Get(Form("%s/gEffi_%s",dirname.Data(),graphname.Data())); 
  TGraph* otherGraph = (TGraph*) filo->Get(Form("%s/gOther_%s",dirname.Data(),graphname.Data())); 
  if (nobsGraph == nullptr || nbkgGraph == nullptr || npotGraph == nullptr || effiGraph == nullptr || otherGraph == nullptr){
    std::cout << "StatisticalTreatment: SetObservablesFromFile -> Cannot retrieve nobs,nbkg,npot graph " << dirname.Data()<<"/gNObs_"<<graphname.Data() << " from " << filename.Data() << " " << nobsGraph << " " << nbkgGraph << " " << npotGraph << " " << effiGraph << " " << otherGraph << endl;
    exit(1);
  }
  ...
  for (int i=0; i< nobsGraph->GetN(); i++) {
    ...
    for (int q=0; q<3; q++) {
      for (int i=0; i<2; i++) fObservables.BkgPerPOTVsSqrtsParObs[q][i] = otherGraph->GetY()[12+i+2*q];
    }
  }
  fObservables.POTSlopeCorrectionObs  = otherGraph->GetY()[18];//CHECK
...
//				 (int)fMassMax,(int)((fMassMax-(int)fMassMax)*1000. + 0.5));
  ...
  fLBHisto->Write();
  for (uint k=0; k<2; k++){ // 90% CL, 95%CL
    for (uint i=0; i<2; i++){ // 1sigma, 2sigma
      fAsimov_a[k].at(i)->Write();
    }
  }
  for (uint im = 0; im < (uint) fNMassBins; im++){
    ...
    fQobs_a.at(im)->Write();
    fQmuProfile.at(im)->Write();
    fPprimeProfile.at(im)->Write();
  }
  ...
  // pull histograms on Btoy: B-fits and SB-fits
  for (int pseudo=0; pseudo<2; pseudo++){
    for (uint ip=0; ip < fPullBFitOnBToy[pseudo].size(); ip++)  fPullBFitOnBToy[pseudo].at(ip)->Write();
   
    for (uint im = 0; im < (uint) fNMassBins; im++){
      for (uint ip=0; ip < fPullSBFitOnBToy[pseudo].size()/fNMassBins; ip++)  fPullSBFitOnBToy[pseudo].at(im+ip*fNMassBins)->Write();
    }
  }

  
  for (uint im = 0; im < (uint) fNMassBins; im++){
    for (uint ip=0; ip < fPullBFitOnSBToy.size()/fNMassBins; ip++)  fPullBFitOnSBToy.at(im+ip*fNMassBins)->Write();
  }
  
  for (uint im = 0; im < (uint) fNMassBins; im++){
    for (uint ip=0; ip < fPullSBFitOnSBToy.size()/fNMassBins; ip++)  fPullSBFitOnSBToy.at(im+ip*fNMassBins)->Write();
  }

  // fit results graphs on Btoy: B-fits and SB-fits

  fGraphBFitOnBToy->Write();   
  for (uint i=0; i < fGraphSBFitOnBToy.size(); i++) fGraphSBFitOnBToy.at(i)->Write();
...
double statisticalTreatmentTH::PhiFun(double x){
  // 1-Phi(x) = (1.-TMath::Erf(x/sqrt(2.)))/2.
  // Phi(x) = 1/2+1/2TMath::Erf(x/sqrt(2.))
  return (1.+TMath::Erf(x/TMath::Sqrt(2.)))*0.5;
}
double statisticalTreatmentTH::PhiFunInv(double x){
  // 1-Phi(x) = (1.-TMath::Erf(x/sqrt(2.)))/2.
  // Phi(x) = [1+TMath::Erf(x/sqrt(2.))]/2
